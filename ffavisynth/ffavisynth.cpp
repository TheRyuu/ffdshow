...
public:
    ...
    inline AVS_Value avs_new_value_clip(AVS_Clip * v0) {
        AVS_Value v;
        avs_set_to_clip(&v, v0);
        return v;
    }
    inline AVS_VideoFrame* avs_new_video_frame(AVS_ScriptEnvironment * env,  const AVS_VideoInfo * vi) {
...
public:
    ...
    AVS_VideoFrame* GetFrame(AVS_FilterInfo *p, int frame) {
        ...
        if (avs_is_planar(&p->vi)) {
            src[0] = avs_get_read_ptr_p(srcframe, AVS_PLANAR_Y);
            src[1] = avs_get_read_ptr_p(srcframe, AVS_PLANAR_U);
            src[2] = avs_get_read_ptr_p(srcframe, AVS_PLANAR_V);
            srcStride[0] = avs_get_pitch_p(srcframe, AVS_PLANAR_Y);
            srcStride[1] = avs_get_pitch_p(srcframe, AVS_PLANAR_U);
            srcStride[2] = avs_get_pitch_p(srcframe, AVS_PLANAR_V);
            dst[0] = avs_get_write_ptr_p(dstframe, AVS_PLANAR_Y);
            dst[1] = avs_get_write_ptr_p(dstframe, AVS_PLANAR_U);
            dst[2] = avs_get_write_ptr_p(dstframe, AVS_PLANAR_V);
            dstStride[0] = avs_get_pitch_p(dstframe, AVS_PLANAR_Y);
            dstStride[1] = avs_get_pitch_p(dstframe, AVS_PLANAR_U);
            dstStride[2] = avs_get_pitch_p(dstframe, AVS_PLANAR_V);
        } else {
    ...
    static void wfmt2vi(const WAVEFORMATEX &wfx, AVS_VideoInfo &vi) {
        ...
        if (wfx.wFormatTag == WAVE_FORMAT_PCM)
            switch (wfx.wBitsPerSample) {
                case  8:
                    vi.sample_type = AVS_SAMPLE_INT8 ;
                    break;
                case 16:
                    vi.sample_type = AVS_SAMPLE_INT16;
                    break;
                case 24:
                    vi.sample_type = AVS_SAMPLE_INT24;
                    break;
                case 32:
                    vi.sample_type = AVS_SAMPLE_INT32;
                    break;
            }
        else {
