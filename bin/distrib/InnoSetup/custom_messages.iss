...
fr.FinishedRestartLabel=Pour completer l'installation de [name], l'installation doit redemarrer votre ordinateur. Voulez-vous le redemarrer maintenant ?%n%nHint : Pour éviter de redemarrer votre ordinateur la prochaine fois, il est conseiller d'activer l'option "ne pas utiliser ffdshow dans :"-"explorer.exe" dans « Infos & Debogage » du panneau configuration du décodeur video ffdshow.
; Russian
...
fr.SelectPluginDirLabel2=Où doit-être installé le plugin %1 ?
fr.SelectPluginDirLabel3=Sélectionner le dossier où doit être installé le plugin %1, et appuyer sur suivant.
...
fr.spk_HRTF=Fonction de transfert relier à la tête (HRTF)
...
fr.comp_SetupLabelV2=Parce que quelques jeux se plantent ou gèlent quand ffdshow est installé,%nNous conseillons de sélectionner "Utiliser ffdshow seulement dans".
fr.comp_SetupLabelV3=Lister le nom d'exécutables qui sont compatible avec le décodeur video ffdshow, un pour chaque ligne.
fr.comp_SetupLabelA1=Administration des problemes de compatibilité (AUDIO)
