Polski

============================================
Translated by Virtual_ManPL (Artur OsiÒski)
e-mail: Virtual_ManPL@wp.pl
Gadu-Gadu: 6361956
Jabber: Virtual_ManPL@maszyna.pl
Latest translated ffdshow : 1589
Revision: 2
============================================

[115]
0=Kodeki

[182]
1162=Resetuj
1163=Pomoc
1723=Resetuj u≥oøenie

[192]
2=Anuluj

[101]
2=Anuluj
3=Zastosuj
1835=Eksportuj ustawienia...
1836=WspomÛø

[-1]
Decoder=Dekoder
Details=SzczegÛ≥y
disabled=Wy≥πczony
High accuracy mode is enabled for tremor.=Tryb wysokiej dok≥adnoúci jest w≥πczony dla termor.
enabled=W≥πczony
FLAC (Free Lossless Audio Codec);To play .flac files you also need a source filter.=FLAC (Free Lossless Audio Codec); By odtwarzaÊ pliki .flac potrzebujesz rÛwnieø filtra ürÛd≥owego.
TTA (Lossless Audio Codec);To play .tta files you also need a source filter.=TTA (Lossless Audio Codec); By odtwarzaÊ pliki .tta potrzebujesz rÛwnieø filtra ürÛd≥owego.
Other ADPCM=Inne ADPCM
all supported=Wszystkie obs≥ugiwane
Adaptive Multi-Rate in 3gp files (AMR)=Adaptive Multi-Rate w plikach 3gp (AMR)
not WMA9 pro, voice or lossless=Nie dla WMA9 pro, voice or lossless
AVIS (AviSynth AVI files)=AVIS (pliki AviSynth AVI)
Uncompressed=Nieskompresowany
List of applications for which ffdshow should not load, separated by semicolons=Lista aplikacji dla ktÛrych ffdshow nie ≥aduje siÍ, oddzielone drukropkiem
List of applications which are compatible with ffdshow, separated by semicolons.=Lista aplikacji ktÛre sπ kompatybilne z ffdshow, oddzielone dwukropkiem.
very high=Najwyøszy
ffdshow default=Domyúlnie
preferred=Preferowany
normal=Normalny
unlikely=Niski
do not use=Najniøszy
no limitations=W≥πczone - brak limitÛw
only one - check previous filter only=Tylko jedno-sprawdü tylko poprzednie filtry
only one - check all previous filters=Tylko jedno-sprawdü wszystkie poprzednie filtry
only one - check all filters in graph=Tylko jedno-sprawdü wszystkie filtry na wykresie
none - disabled=Wy≥πczone
ATSC A-52 stream decoder=Dekoder strumienia ATSC A-52
DTS Coherent Acoustics stream decoder=Dekoder strumienia DTS Coherent Acoustics
Windows Media Audio compatible decoder=Kompatybilny dekoder Windows Media Audio
DSP Group TrueSpeech compatible decoder=Kompatybilny dekoder DSP Group TrueSpeech
QDM2 compatible decoder (incomplete)=Kompatybilny dekoder QDM2 (niedokoÒczony)
IMC compatible decoder=Kompatybilny dekoder IMC
ATRAC3 compatible decoder=Kompatybilny dekoder ATRAC3
Source file=èrÛd≥o pliku
Input description=Parametry wejúciowe
Output description=Parametry wyjúciowe
Output speakers=Tryb g≥oúnikÛw
Current input bitrate=Aktualna wejúciowa szybkoúÊ przesy≥ania danych
Host application=G≥Ûwna aplikacja
Source file name=Nazwa pliku ürÛd≥owego
Sample frequency=CzÍstotliwoúÊ prÛbki
Number of channels=IloúÊ kana≥Ûw
Sample format=Format prÛbki
CPU load=Obciπøenie procesora
System time=Czas systemowy
Movie length=D≥ugoúÊ filmu
Encoder info=Informacja kodeka
Short configuration description=KrÛtki opis konfiguracji
File where to write statistics=Plik gdzie zapisujπ siÍ statystyki
ffdshow audio decoder configuration=Konfiguracja dekodera düwiÍku ffdshow
Thank you!=DziÍkujÍ Tobie!
Set all stable formats to libavcodec=Ustaw wszystkie stabilne formaty na libavcodec
Set all supported formats to Xvid=Ustaw wszystkie obs≥ugiwane formaty na XviD
Maximize SPDIF compatibility=KompatybilnoúÊ S/PDIF
Dynamic range compression=Dynamiczny zakres kompresji
Check for DTS in WAV=Sprawdü DTS w WAV
Use SPDIF when AC3 output set=Uøyj S/PDIF gdy jest ustawiony AC3
Preset name=Nazwa
left=W lewo
top=w gÛrÍ
Reset=Resetuj
Edit=Edytuj
New=Nowy
Save as...=Zapisz jako...
Rename preset...=ZmieÒ nazwÍ...
Delete preset=UsuÒ
Show on startup=Pokazuj na starcie
From default=Z domyúlnego
From selected=Z zaznaczonego
From file...=Z pliku...
Load ffdshow preset=Za≥aduj aktualne ffdshow
Rename=ZmieÒ nazwÍ
Insert file name=Wstaw nazwÍ pliku
Insert exe file name=Wstaw nazwÍ pliku exe
Preset '%s' will be autoloaded ...=Aktualne '%s' zostanie automatycznie za≥adowane...
on movie file name match with preset name=dopasowanie nazwy filmu z aktualnπ nazwπ
on movie file name match (with wildcards)=dopasowanie wymiaru filmu (z dzikπ kartπ)
on application exe file name match=dopasowanie nazwy pliku exe
on volume name match=dopasowanie nazwy
Names of DirectShow filters, wildcard allowed=Nazwy filtrÛw DirectShow, dzika karta dozwolona
on number of channels match=dopasowanie iloúci kana≥Ûw
on sampling frequency match=dopasowanie czÍstotliwoúci prÛbki
on volume serial match=dopasowanie kolejnego wolumenu
on decoder match=dopasowanie dekodera
on a DirectShow filter presence=w czasie obecnoúci filtrÛw DirectShow
Preset autoload condition help=Aktualne warunki automatycznego za≥adowania pomocy
none=Wy≥πczona
light=Ma≥a
medium=årednia
heavy=Duøa
mute=Wycisz
solo=Solo
Channel=Kana≥
File=Plik
front left=Przedni lewy
front right=Przedni prawy
front center=Przedni centralny
back left=Tylni lewy
back right=Tylni prawy
invalid=nieprawid≥owy
Winamp2 application directory, not plugin directory.=Katalog aplikacji Winamp 2, a nie katalog wtyczek.
Select Winamp 2 directory (not the plugins directory!)=Wybierz katalog Winamp'a 2 (nie katalog z wtyczkami!)
lowpass=Dolnoprzepustowy
highpass=GÛrnoprzepustowy
bandpass=årodkowoprzepustowy
bandstop=årodkowozaporowy
Select impulse file=Wybierz plik z impulsami
higher than=wiÍksze od
lower than=mniejsze niø
libavcodec normal quality=libavcodec normalna jakoúÊ
libavcodec high quality=libavcodec wysoka jakoúÊ
libavcodec highest quality=libavcodec najwyøsza jakoúÊ
libsamplerate sinc high quality=libsamplerate sinc wysoka jakoúÊ
libsamplerate sinc medium quality=libsamplerate sinc úrednia jakoúÊ
libsamplerate sinc fastest=libsamplerate sinc najszybciej
back center=Tylni centralny
same as input=Identyczny jak wejúciowy
headphone virtual spatialization=Wirtualne przestrzenne s≥uchawki
Library=Biblioteka
Version=Wersja
"not needed": no custom channel mapping="nie potrzebny": bez w≥asnego mapowania kana≥Ûw
Useful for directly storing encoded ac3 to a file in graphedt with File Writer filter=Potrzebny do bezpoúredniego gromadzenia kodowanego AC3 dla pliku w GraphEdit z filtrem File Writer
any filter=Kaødym filtrem
ffdshow audio decoder=ffdshow dekoder düwiÍku
H264, X264, VSSH (incomplete), DAVC, PAVC, AVC1=H264, X264, VSSH (niedokoÒczony), DAVC, PAVC, AVC1
Other MPEG4=Inne MPEG4
ffdshow and ffvfw internal FOURCCs=wewnÍtrzny FOURCC ffdshow i ffvfw
H263, L263, M263, U263, X263, S263 (in 3gp files)=H263, L263, M263, U263, X263, S263 (w plikach 3gp)
Supported FOURCCs/remarks=Obs≥ugiwane FOURCC
ffdshow video decoder configuration=Konfiguracja dekodera obrazu ffdshow
all YUV=Wszystkie YUV
all RGB=Wszystkie RGB
8-bit palletized=8 bitowa paleta
Enable in Vista WMP11=W≥πcz w WMP11 w Vista
use speedup tricks=W≥πcz optymalizacjÍ
DVD decoding (not working yet)=dekodowanie DVD
Internal postprocessing=WewnÍtrzne przetwarzanie
skip deblocking when safe=PomiÒ usuwanie blokÛw
skip deblocking always=Zawsze pomiÒ usuwanie blokÛw
Input size and aspect ratio=WielkoúÊ wejúciowa i proporcja obrazu
Input FOURCC=Wejúciowe FOURCC
Movie FPS=IloúÊ klatek na sekundÍ
Input bitrate=Wejúciowa szybkoúÊ przesy≥ania danych
Output colorspace=Wyjúciowa paleta kolorÛw
Current frame=Aktualna klatka
MD5 sum=Suma MD5
Input aspect ratio=Wejúciowa proporcja
Output aspect ratio=Wyjúciowa proporcja
Frame timestamps=Czas poczπtkowy i koÒcowy klatki
Frame duration=Czas trwania klatki
Decoder FPS=Dekodowana szybkoúÊ klatek na sekundÍ
Queued samples=Oczekujπce prÛbki
Video delay=OpÛünienie obrazu
Time on ffdshow=Czas w ffdshow
AviSynth info=Informacja AviSynth
Accurate deblocking=Dok≥adne usuwanie blokÛw
Frame type=Typ klatki
Subtitles delay=OpÛünienie napisÛw
Coded frame size=WielkoúÊ aktualnej klatki
Input colorspace=Wejúciowa paleta kolorÛw
Input size=Wejúciowa wielkoúÊ obrazu
Output size=Wyjúciowa wielkoúÊ obrazu
Frame mean quantizer=Kwantyzator aktualnej klatki
Current playing time=Aktualny czas odtwarzania
Remaining time=Pozosta≥y czas do koÒca
GMC warp points=IloúÊ wektorÛw uøytych w globalnej kompensacji ruchu
Default=Domyúlny
Chinese=ChiÒski
Hebrew=Hebrajski
Arabic=Arabski
Greek=Grecki
Turkish=Turecki
Vietnamese=Wietnamski
Thai=Tajski
Easteurope=Wschodnioeuropejski
Cyrillic=Cyrylica
Baltic=Ba≥tycki
thin=CieÒka
extralight=Ekstra ma≥a
semibold=PÛ≥wyt≥uszczona
bold=Wyt≥uszczona
extrabold=Ekstra wyt≥uszczona
Jitter=Szybkozmienna fluktuacja fazy
3/0 - 3 front=3/0 - 3 przednie kana≥y
3/2 - 5 channels=3/2 - 5 kana≥Ûw
Select Winamp preset file=Wybierz plik z ustawieniami Winamp
MPEG1 codec=MPEG1 kodek
MPEG2 codec=MPEG2 kodek
MPEG in AVI=MPEG w AVI
Windows Media Video 9 Simple & Main Profiles (WMV3)=Windows Media Video 9 Profile Simple & Main (WMV3)
Windows Media Video 9 Advanced Profile (WVC1)=Windows Media Video 9 Profil Advanced (WVC1)
Huffyuv codec for libavcodec (HFYU, FFVH)=Huffyuv kodek dla libavcodec (HFYU, FFVH)
Other MJPEG=Inne MJPEG
FFV1 codec for libavcodec (FFV1)=FFV1 kodek dla libavcodec (FFV1)
Other DV=Inne DV
Chinese AVS video (AVS1-P2, JiZhun profile)=Chinese AVS video (AVS1-P2, profil JiZhun)
Real Video (RV10, RV20, incomplete)=Real Video (RV10, RV20, niedokoÒczony)
Doesn't support 24-bit compression.=Nie obs≥uguje kompresji 24 bitowej.
Glowing shadow=Promienisty
Gradient shadow=Gradientowy
Classic shadow=Klasyczny
Disabled=Wy≥πczony
or=lub
and=i
on FOURCC match=dopasownie FOURCC
on FOURCC from previous filter match=dopasownie FOURCC z poprzedniego filtra
Useful when FFDShow is in raw mode, the compressed format (if any) is given by the previous filter in the chain.=Uøyteczne gdy ffdshow jest w trybie raw, skompresowany format (jeúli jest) jest uøyty przez poprzedni filter w szeregu.
on pixel aspect ratio match=dopasownie proporcji pikseli
on picture aspect ratio match=dopasownie proporcji obrazu
on frame rate match=dopasownie tempa klatek
Action=Akcja
Key=Klawisz
Activation key 1=Klawisz aktywacji 1
Activation key 2=Klawisz aktywacji 2
Second function=Druga funkcja
Seek forward=Szukaj w przÛd
Weight - most fonts supports only small subset of listed weights.=WielkoúÊ- wiÍkszoúÊ czcionek obs≥uguje tylko ma≥e wielkoúci z listy wielkoúci.
Spacing - distance between characters.=OdstÍp - odleg≥oúÊ miedzy czcionkπ.
Works when the video has non-square pixel aspect ratio.=Dzia≥a kiedy obraz ma nie kwadratowe piksele wspÛ≥czynnika.
right arrow=Strza≥ka w prawo
Seek backward=Szukaj w ty≥
left arrow=Strza≥ka w lewo
Toggle OSD=Prze≥πcznik OSD
Toggle visualizations=Prze≥πcznik wizualizacji
Toggle crop/zoom=Prze≥πcznik kadrowania
Toggle deinterlace=Prze≥πcznik przeplotu
Toggle levels=Prze≥πcznik leweli
Toggle noise=Prze≥πcznik szumu
Toggle picture properties=Prze≥πcznik w≥aúciwoúci
Toggle postprocessing=Prze≥πcznik przetwarzania
Toggle blur=Prze≥πcznik rozmywania
Toggle resize=Prze≥πcznik rozmiaru
Toggle subtitles=Prze≥πcznik napisÛw
Toggle sharpen=Prze≥πcznik wyostrzania
Toggle flip=Prze≥πcznik odbicia symetrycznego
Toggle avisynth=Prze≥πcznik AviSynth
closest=AutowybÛr
center=årodek
right=W prawo
blend=Mieszania
darken=Pociemniania
lighten=Rozjaúniania
add=Dodawania
softlight=Pojaúniania
exclusion=Wykluczania
auto=Automatyczna
none (fastest, most ugly)=Brak
approximate=Przybliøona
full (slowest)=Pe≥na
bilinear (fast and not too bad)=Dwuliniowa
swscaler gaussian=Gauss'owski swscaler
all integer=Wszystkie sta≥oprzecinkowe
all float=Wszystkie zmiennoprzecinkowe
8-bit integer=8 bitowy sta≥oprzecinkowy
16-bit integer=16 bitowy sta≥oprzecinkowy
24-bit integer=24 bitowy sta≥oprzecinkowy
32-bit integer=32 bitowy sta≥oprzecinkowy
32-bit float=32 bitowy zmiennoprzecinkowy
64-bit float=64 bitowy zmiennoprzecinkowy
box=Box
triangle=Triangle
hamming=Hamming
hanning=Hanning
blackman=Blackman
flattop=Flattop
kaiser=Kaiser
IDCT(libavcodec)=OdwrotnoúÊ dyskretnej kosinusowej transformaty (libavcodec)
English (may work with other languages)=Angielski (moøe dzia≥aÊ z innymi jÍzykami)
YCbCr <-> RGB conversion options=Opcje konwersji YCbCr <-> RGB
Bob for video source\nWeave for film source\nIn most cases, Weave is the same as unchecked HW deinterlacing.=Bob dla ürÛd≥a obrazu\nWeave dla ürÛd≥a filmu\nW wiÍkszoúci przypadkÛw, w≥πczony Weave dzia≥a tak samo jak wy≥πczone sprzÍtowe usuwanie przeplotu.
Connect only to:\n  Overlay Mixer\n  VMR\n  VMR9\n  VobSub\n  Haali's Video Renderer\n  EVR\n  ffdshow=Po≥πcz tylko z:\n  Mikserem nak≥adki\n  Miksujπcym rendererem obrazu 7\n  Miksujπcym rendererem obrazu 9\n  VobSub\n  Rendererem obrazu Haali\n  Ulepszonym miksujπcym rendererem obrazu\n  ffdshow
Unchecked: Use classic connection method.\nChecked: Use an advanced connection method that includes pixel aspect ratio (SAR) and interlacing information when connecting with the next filter (usually a video renderer).\nIndeterminate/grayed: Try advanced connection method first and fall back to classic connection method if it fails.\n\n(formerly called "Use overlay mixer")=Niezaznaczony: Uøyj klasycznej metody po≥πczenia.\nZaznaczony: Uøyj zaawansowanej metody po≥πczenia ktora zawiera wspÛ≥czynnik pikseli (SAR) oraz informacje o przeplocie kiedy ≥πczy siÍ z nastÍpnym filtrem (zazwyczaj z rendererem obrazu).\nWpÛ≥zaznaczony: Pierw uøyj zaawansowanej metody i jeúli zawiedzie uøyj klasycznej metody\n\n(Formalnie zwana nak≥adkπ obrazu)
Send interlacing related information obtained from the input stream or ffdshow's internal decoders to the next filter. Some filters (like video renderers) will use this information to deinterlace the video if neccessary.\nThis is just for informing the downstream filters - the actual result will depend purely on the implementation of these filters.\n\n(formerly called "HW deinterlacing")=Wyúlij informacjÍ zwiπzanπ z przeplotem uzyskanπ od wejúciowego strumienia lub wewnÍtrznych dekoderÛw ffdshow do nastÍpnego filtra. NiektÛre filtry (jak renderery obrazu) w razie potrzeby uøywajπ tej informacji do usuwania blokÛw w obrazie.
indeterminate state - connect to any filter, allows output format changes only if connected to supported filter.\n\nFilters known to support dynamic format change:\n  Overlay Mixer\n  VMR\n  VMR9\n  VobSub\n  Haali's Video Renderer\n  EVR\n  ffdshow=WpÛ≥zaznaczony: po≥πcz do kaødego filtra, pozwÛl na zmianÍ wyjúciowego formatu tylko jeúli po≥πczono z obs≥ugiwanym filtrem.\n\nZnane filtry ktÛre obs≥ugujπ dynamicznπ zmianÍ formatu\n  Mikserem nak≥adki\n  Miksujπcym rendererem obrazu 7\n  Miksujπcym rendererem obrazu 9\n  VobSub\n  Rendererem obrazu Haali\n  Ulepszonym miksujπcym rendererem obrazu\n  ffdshow
constant=sta≥e
mpeg1/2 decoder only=Tylko dekoder mpeg1/2
Error=B≥πd
bottom=årodek
guess MVS=Przypuszczalne wektory ruchu
deblock=Usuwanie blokÛw
guess MVS + deblock=Obie wyøej wymienione metody
simple MMX=prosta MMX
simple=prosta
integer=ca≥oúciowa
No quantization matrices available.=Matryce kwantyzatora sπ niedostÍpne.
careful=Ostroøna
compliant=Uleg≥a
aggressive=Agresywna
very aggressive=Najagresywniejsza
default=Domyúlny
(Afan) Oromo=Oromo
Abkhazian=Abchaski
Albanian=AlbaÒski
Amharic=Amharski
Aragonese=AragoÒski
Armenian=OrmiaÒski
Assamese=Assamski
Avaric=Awarski
Avestan=Awestyjski
Aymara=Ajmara
Azerbaijani=Azerski
Bashkir=Baszkirski
Basque=Baskijski
Belarusian=Bia≥oruski
Bengali; Bangla=Bengalski
Bosnian=Boúniacki
Breton=BretoÒski
Bulgarian=Bu≥garski
Burmese=BirmaÒski
Cambodian=Khmerski
Catalan=KataloÒski
Corsican=KorsykaÒski
Czech=Czeski
Dansk=DuÒski
Deutsch=Niemiecki
English=Angielski
Espanol=HiszpaÒski
Estonian=EstoÒski
Faroese=Farerski
Fijian=Fidøyjski
Finnish=FiÒski
French=Francuski
Frisian=Fryzyjski
Galician=Galicyjski
Georgian=GruziÒski
Greenlandic=Grenlandzki
Gujarati=Gudüarati
Hrvatski=Chorwacki
Hungarian=WÍgierski
Icelandic=Islandzki
Indonesian=Indonezyjski
Irish=Irlandzki
Italian=W≥oski
Japanese=JapoÒski
Javanese=Jawajski
Kashmiri=Kaszmirski
Kazakh=Kazachski
Kinyarwanda=Ruanda-rundi
Kirghiz=Kirgiski
Korean=KoreaÒski
Kurdish=Kurdyjski
Laothian=LaotaÒski
Latin=£aciÒski
Latvian, Lettish=£otewski
Lithuanian=Litewski
Macedonian=MacedoÒski
Malagasy=Malgaski
Malay=Malajski
Malayalam=Malajalam
Maltese=MaltaÒski
Maori=Maoryski
Moldavian=Mo≥dawski
Mongolian=Mongolski
Nauru=NauruaÒski
Nederlands=Niderlandzki
Nepali=Nepalski
Norsk=Nowonorweski
Norwegian=Norweski
Occitan=OksytaÒski
Oriya=Orija
Pashto, Pushto=Paszto
Persian=Perski
Polish=Polski
Portugues=Portugalski
Punjabi=Pendøabski
Quechua=Keczua
Rhaeto-Romance=RetroromaÒski
Romanian=RumuÒski
Russian=Rosyjski
Samoan=SamoaÒski
Sanskrit=Sanskryt
Scots Gaelic=Szkocki Gealicki
Serbian=Serbski
Serbo-Croatian=Serbsko-Chorwacki
Sinhalese=Syngaleski
Slovak=S≥owacki
Slovenian=S≥oweÒski
Somali=Somalijski
Sundanese=Sundajski
Swedish=Szwedzki
Tamil=Tamilski
Tatar=Tatarski
Tibetan=TybetaÒski
Tigrinya=Tigrinia
Turkmen=TurkmeÒski
Uighur=Ujgurski
Ukrainian=UkraiÒski
Uzbek=Uzbecki
Welsh=Walijski
Yiddish=Jidysz
Yoruba=Joruba
subtitle=napisÛw
line=linii
character=znakÛw
End-of-line word wrapping=Przenoszπc koÒcowe wyrazy do nowej linijki
Smart wrapping, lines are evenly broken=Dzielπc je po rÛwno
Smart wrapping, lower line gets wider=Poszerzajπc niøszπ linijkÍ
Load subtitles file=Za≥aduj plik z napisami
Very incomplete and experimental.\nSend me samples which don't work.=Opcja niekompletna i eksperymentalna.\nWyúlij mi prÛbki z ktÛrymi nie dzia≥a.
ffdshow searches subtitle files in the folders which are configured in the edit box above.\nFor video.avi, ffdshow searches video.utf, video.idx, video.sub,... and use the file which is found at the first time.\nEnumerate extensions in the order you like and separate them by semicolons.\n\nutf;idx;sub;srt;smi;rt;txt;ssa;aqt;mpl;usf is the default settings.=ffdshow przeszuka pliki z napisami w folderze ktÛry zosta≥ ustawiony w opcji wyøej.\nDla pliku NAZWA.avi, ffdshow przeszuka pliki NAZWA.utf, NAZWA.idx, NAZWA.sub... oraz nastÍpnie uøyje pliku ktÛry ma wiÍkszy piorytet w opcji.\n Zapisuj rozszerzenia plikÛw z napisami w kolejnoúci w ktÛrej maja byÊ przeszukiwane oraz oddzielaj je przez znak ;\n\nutf;idx;sub;srt;smi;rt;txt;ssa;aqt;mpl;usf taka kolejnoúÊ jest domyúlna.
4x4 luma=4x4 Luminancja
4x4 chroma=4x4 Chrominancja
Smooth timestamps=P≥ynne oznaczenia czasu
Ignore pulldown=Ignoruj pulldown
Apply pulldown=ZezwÛl na pulldown
Number of frames to buffer ahead=IloúÊ klatek buforowanych z wyprzedzeniem
Number of used frames to keep in buffer=IloúÊ zatrzymywanych klatek w buforze
Select DScaler filter=Wybierz filtr DScaler
None=Wy≥πczony
Linear=Linearna
Cubic=Szeúcienna
Number of taps:=IloúÊ taps:
gray=SzaroúÊ
color=Kolor
opacity=Przeü.
size=Rozmiar
frequency=CzÍstotliwoúÊ
amplitude=Amplituda
duration=Czas trwania
chroma blur:=Chrominancja:
radius:=ZasiÍg:
luma blur:=Luminancja:
luma:=luminancja
chroma:=chrominancja
time:=czas
original=Oryginalny
off=Wy≥πczona
curve=Wykres
No=Brak
XY=Poziom-Pion
Solid fill=Sta≥e wype≥nienie
Shape XY=Wyraüny Poziom-Pion
Over video=Na obrazie
Over top letterbox=Na gÛrnej skrzynce
Over top & left letterbox=Na gÛrnej i lewej skrzynce
Over top & right letterbox=Na gÛrnej i prawej skrzynce
Over bottom letterbox=Na dolnej skrzynce
Over bottom & left letterbox=Na dolnej i lewej skrzynce
Over bottom & right letterbox=Na dolnej i prawej skrzynce
Over left letterbox=Na lewej skrzynce
Over right letterbox=Na prawej skrzynce
Direct=Bezpoúrednio
Opposite=Przeciwstawnie
Interpolation=Interpolacyjnie
Select name of exported Girder file=Wybierz nazwÍ dla pliku eksportowanych komend
Toggle warpsharp=Prze≥πcznik Warpsharp
Grab frame=Zrzut klatki
Subtitles delay/size decrease=Zmniejszenie opÛünienia/wielkoúci napisÛw
Subtitles delay/size increase=ZwiÍkszenia opÛünienia/wielkoúci napisÛw
Subtitles position decrease=Zmniejszenie pozycji napisÛw
Subtitles position increase=ZwiÍkszenie pozycji napisÛw
Decrease video delay=Zmniejszenie opÛünienia obrazu
Increase video delay=ZwiÍkszenie opÛünienia obrazu
Previous preset=Poprzedni profil
Next preset=NastÍpny profil
Luminance sharpening:=Wyostrzanie luminancji:
Chroma sharpening:=Wyostrzanie chrominancji:
Unsharp masking threshold:=Si≥a wyostrzajπcej warstwy:
Adaptive sharpenning strength:=Si≥a wyostrzania:
independent=WyrÛwnana niezaleønie
downsampled=WyrÛwnana w dÛ≥
high quality 3-pass=Najlepsza - 3 stopniowa
fast 3-pass=årednia - 3 stopniowa
fast 1-pass=S≥aba - 1 stopniowa
Cubic interpolation=Przekszta≥cenia szeúciennego
Bypass=Obejúcia
Linear interpolation=Przekszta≥cenia linearnego
Linear blending=Mieszania linearnego
Cubic blending=Mieszania szeúciennego
Median=Mediany
Framerate doubler=Podwajania szybkoúci klatek
ffmpeg deinterlacer=Usuwanie przeplotu ffmpeg
Kernel deinterlacer=Usuwanie przeplotu Kernel'a
Tolerance when scanning bars=Tolerancja podczas skanowania paskÛw
Scan of bars will stop after this position. Set 0 to always scan for bars=Skanowanie paskÛw zakoÒczy siÍ po tym po≥oøeniu. Ustaw 0 by zawsze skanowaÊ paski
Set the delay in ms to rescan for the bars=Ustaw opÛüniene w ms do ponownego przeskanowania paskÛw
greater than=wiÍksza od
less than=mniejsza niø
Warped resize=ZwiÍkszenie Warped
Fast bilinear=Szybka dwuliniowa
Bilinear=Dwuliniowa
Bicubic=Dwuszeúcienna
Point=Punktowa
Area=Obszarowa
Simple resize=Prosty
Active preset name=Obecna aktywna nazwa
side left=Boczny lewy
side right=Boczny prawy

[116]
0=DirectShow
1285=Ustawienia DirectShow
1503=Piorytet:
1502=Wielokrotne rzπdanie ffdshow
1935=Nie uøywaj ffdshow w:
1970=Uøywaj ffdshow tylko w:
1973=Edytuj...
1974=Edytuj...
2051=Pokazuj informacjÍ kiedy nieznana aplikacja prÛbuje za≥adowaÊ ffdshow

[2020]
1284=Informacja
0=Informacja
1832=Uøyj nastÍpujπcych instrukcji SIMD (jeúli sπ obs≥ugiwane przez CPU)

[141]
1774=Poziome po≥oøenie:
1776=Pionowe po≥oøenie:
1493=Zapisz w:
1491=W≥asne:

[136]
0=Tray, komunikat, úcieøki
1257=Ustawienia ikony Tray
2016=Brak
2017=Nowe
2018=Stare
1074=Rozszerzona informacja w tray
1073=Dodaj ikonÍ menu tray do menu strumieni/napisÛw odtwarzacza
1258=Ustawienia komunikatu
1259=Ustawienia úcieøki
1071=PrzywrÛÊ poprzedniπ pozycjÍ
1072=Pokaø wskazÛwki
1293=JÍzyk:
2019=Ikony Tray:
1143=Ikona Tray\r\n  podwÛjne klikniÍcie otwiera okno konfiguracji\r\n  prawe klikniÍcie otwiera menu z g≥Ûwnymi ustawieniami

[248]
0=StrumieÒ
1709=Prze≥πcznik strumienia
1683=W≥πcz
1710=StrumieÒ

[119]
0=Profile
1016=Nowy
1278=Ustawienia profili
1156=Importuj z pliku...
1155=Eksportuj do pliku
1044=UsuÒ
1157=Warunki automatycznego ≥adowania...
1051=Automatyczne ≥adowanie
1080=PrÛbuj za≥adowaÊ profil z pliku
1045=ZmieÒ

[171]
1212=Aktualne '%s' zostanie automatycznie za≥adowane...
1277=<= wysokoúÊ <=
1276=<= szerokoúÊ <=
1211=dopasowanie wymiaru filmu
8=Zamknij
1888=Oddziel wielokrotne elementy przez znak ;
1915=i
2207=gdy przynajmniej 1 warunek jest spe≥niony (OR)
2208=gdy wszystkie warunki sπ spe≥nione (AND)

[233]
0=Filtry
1473=Ukryte filtry:
1474=DostÍpne filtry:

[259]
1579=Obs≥ugiwane wejúciowe formaty prÛbek
1578=Konwersja przetwarzania zmiennoprzecinkowego na sta≥oprzecinkowe
1582=Redukcja szumu:
0=Wejúcie
1580=16 bitowe sta≥oprzecinkowe
1585=32 bitowe sta≥oprzecinkowe
1586=32 bitowe zmiennoprzecinkowe
1583=Rozpraszanie szumu

[244]
0=Dolby
1657=Dekoder Dolby
1656=OpÛünienie tylnych kana≥Ûw:

[188]
0=G≥oúnoúÊ
1377=G≥oúnoúÊ
1375=G≥Ûwna g≥oúnoúÊ:
1457=Normalizuj
1957=Resetuj po szukaniu
1956=D≥ugoúÊBufora
1641=Aktualny:
1637=Max. wzmocnienie:
1683=Pokaø aktualne lewele g≥oúnoúci
1937=Skala w dB

[189]
0=Ekualizer
1378=Korektor graficzny
1826=Ulepszony korektor graficzny
1814=Za≥aduj ustawienia z Winamp...
1401=Zak≥Ûcenia
1402=Zak≥Ûcenia
1403=Zak≥Ûcenia
1404=Zak≥Ûcenia
1405=Zak≥Ûcenia
1406=Zak≥Ûcenia
1407=Zak≥Ûcenia
1408=Zak≥Ûcenia
1409=Zak≥Ûcenia
1410=Zak≥Ûcenia
1465=dB
1466=dB

[246]
0=FIR
1686=Filtr o skoÒczonej odpowiedzi impulsowej FIR
1696=CzÍstotliwoúÊ:
1700=Max. czÍstotliwoúÊ
1688=IloúÊ uderzeÒ:
1690=Typ filtra:
1692=Typ wykresu:
1698=Zakres:
1683=Pokaø widmo
1143=To jest bardzo nowa opcja dlatego teø opis oraz funkcjonalnoúÊ moøe byÊ niew≥aúciwa.

[262]
1891=Autodostosowanie
1893=Poziom dostosowania:
1889=Si≥a miksujπca:
1881=Plik z impulsami:
1905=Manualne mapowanie kana≥u
1897=Zak≥Ûcenia
1902=Wybierz plik...
1903=WyczyúÊ

[247]
0=Szum
1706=Redukcja szumu
1704=PrÛg:

[190]
1401=Kliknij by zaznaczyÊ katalog Winamp 2...
1404=Konfiguruj
1989=PozwÛl na wielokana≥owe przetwarzanie
1400=Winamp 2 DSP

[236]
1485=WielkoúÊ pokoju:
1487=T≥umienie:
1490=Poziom mokroúci:
1019=Poziom suchoúci
1038=Zakres:
1484=Pog≥os
0=Pog≥os

[249]
0=KrystalicznoúÊ
1711=KrystalicznoúÊ
1712=Rozszerzenie zakresu:
1714=Filtrowanie gÛrnoprzepustowe:
1716=Wzmacniacz harmonizacji:
1718=G≥oúnoúÊ echa:
1720=PowtarzalnoúÊ echa:
1722=Uwydatnienie stereo:

[237]
0=PrÛbkowanie
1497=Zmiana prÛbkowania prÛbki
1498=PrÛbkowanie do...
1499=Tryb:
1875=Zawsze zmieÒ prÛbkowanie
1876=ZmieÒ prÛbkowanie jeúli...
1877=wiÍksze od
1143=Opcja ta umoøliwia zmianÍ prÛbkowania prÛbki do wybranej wartoúci.

[245]
0=OpÛünienie
1676=OpÛünienie
1658=OpÛünienie lewego kana≥u:
1666=OpÛünienie prawego kana≥u:
1663=OpÛünienie centralnego kana≥u:
1669=OpÛünienie lewego tylnego kana≥u:
1672=OpÛünienie prawego tylnego kana≥u
1675=OpÛünienie kana≥u LFE:

[252]
0=LFE
1733=Przenikanie LFE
1734=CzÍstotliwoúÊ dolnoprzepustowa:
1736=Przyrost:
1741=UsuÒ niskie czÍstotliwoúci dodane do LFE z lewego i prawego kana≥u

[253]
0=Kana≥y
1763=ZamieÒ kana≥y
1749=Przedni lewy
1748=Przedni centralny
1746=Tylni lewy
1670=Tylni centralny
1745=Tylni prawy
1143=Kana≥y mogπ by zamienione przez uøycie Miksera (filtra miksujπcego), ale ten filtr robi to szybciej.
1747=Przedni prawy
2188=Boczny lewy
2191=Boczny prawy

[191]
0=Mikser
1405=Mikser kana≥Ûw
1636=Macierz niestandardowa
1586=Normalizuj macierz
1406=Tryb g≥oúnikÛw:
1701=Mowa:
1588=Kontrola g≥oúnoúci
1637=Uwydatnij stereo
1685=RozpiÍtoúÊ s≥uchawek:
1704=Mowa:

[243]
0=Wyjúcie
1574=16 bitowe sta≥oprzecinkowe
1575=24 bitowe sta≥oprzecinkowe
1576=32 bitowe sta≥oprzecinkowe
1577=32 bit zmiennoprzecinkowe
1573=Obs≥ugiwane wyjúciowe formaty prÛbek
1143=Dla lepszej wydajnoúci na s≥abszych komputerach i dla zachowania kompatybilnoúci ze sterownikami zaleca siÍ zaznaczyÊ tylko 16 bitowy sta≥oprzecinkowy wyjúciowy format prÛbki.\r\nWyjúciowy format prÛbki LPCM jest potrzebny dla düwiÍkowych dekoderÛw DVD takich jak np.: Cyberlink.
1813=ZezwÛl na bezpoúredni zapis wyjúciowy do pliku
1811=Nie uøywa nag≥ÛwkaWAVEFORMATEXTENSIABLE gdy nie jest wymagany
1846=Zastosuj tylko dla wyjúcia S/PDIF
1845=Po≥πcz z:
1985=Koduj tylko multikana≥owe strumienie
1984=Urzπdzenie:
1743=Transmisja:

[102]
0=O programie
1849=Licencja
1839=SzczegÛ≥y wersji
1251=ffdshow dekoder obrazu
1072=wersja ffdshow

[193]
2=Zamknij

[231]
0=Wyjúcie
1910=Profil:
1016=Symetryczne odbicie
1034=Po≥πcz tylko z kompatybilnymi filtrami
1017=PozwÛl na zmianÍ wyjúciowego formatu podczas odtwarzania
1966=Metoda:
1036=Automatyczny wybÛr przestrzeni kolorÛw
1031=Ustaw proporcjÍ pikseli
1033=Ustaw flagÍ przeplotu
1913=YUV:
1912=YUV:
1052=Wysoka jakoúÊ konwersji z YV12 do RGB
1284=Obs≥ugiwane wyjúciowe przestrzenie barw:

[132]
0=Czcionka
1259=Czcionka
1335=AutowielkoúÊ
1827=Szybsza
2081=Zamglona
2060=Obramowana
1114=Typ:
1108=WielkoúÊ:
1116=OdstÍp:
1120=RozciπgniÍcie poziome:
1110=WielkoúÊ:
1124=Kolor:
1981=Kana≥ alfa:
1978=Typ cienia:
2054=CieÒ:
2065=WielkoúÊ, kolor i kana≥ alfa
2088=Automatycznie napraw proporcjÍ
2087=Nadpisz wysokoúÊ w SAA/ASS2
2090=RozciπgniÍcie pionowe:
2053=Zarys:
2052=T≥o:

[113]
0=Napisy
1105=Napisy
1279=Szukaj w:
1183=Plik:
1244=Odúwieø
1126=Pozycja w pionie:
1122=Pozycja w poziomie:
1360=WyrÛwnanie:
1683=Napisy 3D
1280=opÛünienie
1281=PrÍdkoúÊ:
1837=Skanowanie heurystyczne
1414=Obserwuj zmiany plikÛw
1722=G≥Íbia:
1358=Proporcja:
1787=W≥πcz napisy wbudowane w film
1789=W≥πcz obs≥ugÍ napisÛw SSA, ASS oraz ASS2 (opcja eksperymentalna)
1788=Dekoduj zamkniÍte napisy w filmie
1968=Priorytet rozszerzenia:

[111]
1162=Resetuj
1163=Pomoc
1164=PrzetwÛrz ca≥y obraz
1167=Tylko prawπ stronÍ
1723=Resetuj u≥oøenie

[120]
0=Przetwarzanie
1073=Przetwarzanie
1158=Si≥a przetwarzania:
1160=Metoda przetwarzania:
1270=Luminancja
1271=Chroma
1328=Nic's pierwszy
1242=ZasiÍg poziomy:
1243=ZasiÍg pionowy:
1275=Popraw poziom:
1035=Luminancji
1038=Pe≥en zakres
1457=Szybkie usuwanie blokÛw SPP
1459=P≥ynny zasiÍg
1274=Odkszta≥canie:
1273=UsuÒ bloki (V):
1272=UsuÒ bloki (H):
1028=W≥asne:
1027=Standardowe:
1008=Automatyczna kontrola jakoúci
1534=Wysoka dok≥adnoúÊ
1238=mplayer'a

[143]
0=SkrÛty
1202=SkrÛty klawiszowe
1416=Odleg≥oúÊ wyszukania
1418=Odleg≥oúÊ wyszukania (2 opcja):
1472=Zdalna kontrola API
1536=Zdalna wiadomoúÊ ffdshow
1537=W≥asna
1829=Eksportuj komendy...
1539=Akceptuj komunikaty z klawiatury
1330=Pokaø komunikat OSD po naciúniÍciu klawisza
1329=Wykonaj naciúniecie klawisza nawet jeúli nie jest on ustawiony

[127]
0=Wyostrzanie
1065=Wyostrzanie
1217=Wysoka jakoúÊ filtrowania blokÛw
1216=Wysoka jakoúÊ
1019=Si≥a:
1021=PrÛg:
1023=Wyostrzanie blokÛw:
1215=Pokaø maskÍ krawÍdzi
1143=xsharpen\r\n Filtr wykonuje subtelne ale przydatne wyostrzanie. Dzia≥a to na zasadzie\r\n ma≥ej ramki nad kaødπ klatkπ lub nad kaødym úrodkowym pikselem ktÛry\r\n jest pominiÍty lub nieruszony (zaleøy od ustawienia progu) albozmapowany \r\n do jednego z dwÛch najjaúniejszych lub najciemniejszych pikseli w oknie\r\n zaleønie od bliskoúci úrodkowego piksela. Jeøeli úrodkowy piksel jest\r\n zmapowany to rÛwnieø Alfa-Zamglenie z oryginalnπ wartoúciπ piksela\r\n uøywajπcy konfigurowalnego ustawienia si≥y. KoÒcowym efektem jest obraz\r\n ktÛry moøe zawieraÊ szum jednak w bardzo ma≥ym stopniu i filtr stara siÍ\r\n przeciwdzia≥aÊ temu.\r\n\r\nUnsharp mask\r\n Filtr wykorzystuje dobrze znane technikÍ uwydatnienia kontrastu\r\n i wyostrzania. G≥Ûwnym celem jest utworzenie wyg≥adzonego obrazu\r\n oraz oryginalnego obrazu a nastÍpnie odjÍcie ich od siebie. Rezultatem tego\r\n bÍdzie obraz o podwyøszonym kontraúcie.\r\n\r\nmsharpen\r\n Filtr ostrzy waøne krawÍdzie obrazu bez wzmacniania szumu. Dzia≥a\r\n na zasadzie wykrywania krawÍdzi i uøywania Kernela zaostrzajπcego tylko\r\n na obszarze krawÍdzi.\r\n\r\nasharp\r\n Jeden z najlepszych filtrÛw wyostrzajπcych ktÛry daje doskona≥π jakoúÊ\r\n obrazu wraz z w≥πczonym filtrem Przetwarzanie. Najlepszπ jakoúÊ uzyskamy  uøywajπc go z obrazem niskiej jakoúci.\r\n\r\nswscaler\r\n Jeden z najlepszych filtrÛw wyostrzajπcych ktÛry daje doskona≥π jakoúÊ\r\n obrazu wraz z w≥πczonym filtrem Przetwarzanie. Najlepszπ jakoúÊ uzyskamy   uøywajπc go z obrazem wysokiej jakoúci.\r\n\r\nOpis xsharpen, unsharp mask oraz msharpen zosta≥ skopiowany z oryginalnej  dokumentacji filtrÛw AviSynth i VirtualDub\r\n Prawa autorskie Donald Graft\r\n\r\nOpis asharp oraz swscaler zosta≥ napisany przez Artura OsiÒskiego (Virtual_ManPL)

[140]
0=Przeplot
1167=Usuwanie przeplotu
1523=Metoda:
1209=ZamieÒ pola

[125]
0=W≥aúciwoúci
1072=W≥aúciwoúci obrazu
1017=Luminancja (kontrast):
1831=Pe≥en zakres
1018=Luminancja (jasnoúÊ):
1781=Barwa:
1782=Si≥a:
1787=Tylko chrominancja
1026=Nasycenie:
1025=OdcieÒ:
1038=Korekcja gammy RGB:
1037=Korekcja gammy:
1932=Efekt skanu:
1830=Poprawa poziomu luminancji

[134]
0=Kadrowanie
1137=PowiÍkszanie i przycinanie
2091=Autoprzycinanie:
1138=PowiÍkszanie
1139=Przycinanie
1140=Blokuj
1105=Poziome:
1107=Pionowe:
1252=GÛra:
1254=Lewo:
1255=Prawo:
1256=DÛ≥:
2094=Tolerancja:
2096=Odúwieø po:
2101=ZakoÒcz skanowanie po
1143=PowiÍkszanie\r\nPrzycinanie\r\n szerokoúÊ i wysokoúÊ przyciÍtego obrazu sπ zaokrπglone do 8 razy\r\n jeúli podasz z≥π wartoúÊ linia bÍdzie mia≥a czerwone t≥o i wartoúÊ nie bÍdzie\r\n zapisana\r\n uøywajπc tego filtra zapewne bÍdziesz musia≥ teø poprawiÊ wspÛ≥czynnik\r\n obrazu
2098=Pionowe
2099=Poziome
2100=Pionowe i poziome

[142]
0=Poziomy
1170=Poziomy
1707=Typ:
1185=Pe≥en zakres
1179=Tylko luminancja
1184=Pokaø histogram
1909=Autokontrola
1291=Wejúciowe
1267=Wyjúciowe
1189=Roz≥oøenie funkcji:
1181=Korekcja gammy
1933=Za≥aduj wykres...
2109=Czasowy pion
2106=PrÛg pionu:
2103=Max. delta Y:

[242]
0=Zamazywanie
1552=Zamazywanie
1574=Tylko luminancja
1551=Typ:
1542=Lewo:
1544=GÛra:
1546=WysokoúÊ:
1548=SzerokoúÊ:
1854=za≥aduj obraz
1553=Zamglenie:
1555=WysokoúÊ X i Y:
1559=Typ krawÍdzi:
1562=Kierunki i tryby pracy:
1143=Filtr tez umoøliwia zamazywanie wybranego obszaru obrazu.

[139]
0=Wizualizacje
1337=Wizualizacje
1075=Kwantyzatory
1357=Wykres
1074=Wektory ruchu
1143=Wizualizacje urozmaicajπce obraz.

[126]
0=Szum
1038=Dodanie szumu
1067=Stary algorytm
1068=Nowy algorytm (avih)
1054=Py≥ i rysy:
1051=Drøenie:
1052=Linie pionowe:
1050=Migotanie:
1041=Si≥a szumu luminancji:
1049=Si≥a szumu chrominancji:
1069=Algorytm z mplayer'a
1039=Jednolity
1232=Uúredniony
1231=Odbiornikowy

[176]
0=DScaler
1309=úcieøka
1308=Filtr DScaler

[137]
0=PrzesuniÍcie
1939=Symetryczne odbicie
1149=Poziome wyrÛwnanie luminancji:
1151=Pionowe wyrÛwnanie luminancji:
1153=Poziome wyrÛwnanie chrominancji:
1120=Pionowe wyrÛwnanie chrominancji:
1940=Lustrzane odbicie
1148=PrzesuniÍcie luminancji i chrominancji
1143=Filtr ten umoøliwia przesuwnie luminancji oraz chrominancji.

[128]
0=Wyg≥adzanie
1134=Wyg≥adzanie obrazu oraz redukcja szumu
1136=ZmiÍkczanie:
1232=UwzglÍdniaj kolory
1137=P≥ynne wyg≥adzanie:
1138=Wygadzanie luminancji:
1139=Wyg≥adzanie chrominancji:
1140=Odszumianie stopniowe:
1141=P≥ynna redukcja szumu mplayer'a
1142=Rozmywanie poziomÛw
1878=Odszumianie3D
1415=JakoúÊ

[133]
0=Rozmiar
1083=Rozmiar i proporcja
1189=ZwiÍksz rozmiar jeúli...
1287=X
1288=Y
1191=ZwiÍksz rozmiar jeúli iloúÊ pikseli...
1188=Zawsze zwiÍksz rozmiar
2086=ZwiÍksz do rozdzielczoúci na pulpicie
1289=Korekcja wspÛ≥czynnika obrazu:
1100=Wy≥πczona
1101=Oryginalna
1102=W≥asna:
1234=WspÛ≥czynnik nak≥adki:
1187=Rozszerz do wielokrotnoúci:
1540=Zwielokrotnij do
1986=Konkretny rozmiar H oraz V
1987=Konkretny poziomy rozmiar:
1285=Rozmiar:
1186=Konkretny wspÛ≥czynnik:
1988=WewnÍtrzne przetwarzanie wspÛ≥czynnika
1286=Aspekt:
2084=PAR wyjúciowego sprzÍtu:

[177]
0=Perspektywa
1317=Korekcja perspektywy
1321=èrÛd≥owe prostokπty
1319=Interpolacja:
1143=Ten filtr jest w trakcie tworzenia dlatego na obrazie mogπ pojawiaÊ siÍ czarne kropki.

[138]
0=Metoda
1892=Zablokuj
1285=Algorytm lumy:
1286=Algorytm chromy
1282=Ustawienia rozmiaru:
1019=Wyostrzanie luminancji:
1020=Wyostrzanie chrominancji:
1360=Z przeplotem
1953=Wysoka dok≥adnoúÊ
1098=Zamglenie luminancji:
1099=Zamglenie chrominancji:
1100=Poziome odkszta≥cenie:
1022=Pionowe odkszta≥cenie:
1143=Algorytmy zwiÍkszania rozmiaru obrazu:\r\n najdok≥adniejszymi sπ algorytmy Lanczos oraz Spline\r\n\r\nZ przeplotem:\r\n zaznaczone - klatki sπ traktowane jako progresywne\r\n niezaznaczone - klatki traktowane sπ jako klatki z przeplotem\r\n wpÛ≥zaznaczone - decyduje typ wejúciowej klatki (uøywane podczas   przetwarzania obrazu MPEG2)

[254]
0=Tekst
1775=pikseli
1788=Poprawiaj "I" oraz "i"
1790=Poprawiaj interpunkcjÍ (np.: "?." -> "?" ; "!." -> "!" itd.)
1795=Poprawiaj gramatykÍ
1791=Poprawiaj wyrazy pisane ca≥oúciowo tylko wielkπ literπ
1792=Usuwaj niechciane spacje pomiÍdzy cyframi
1798=Usuwaj napisy düwiÍkowe (np.: "bum", "bach", "a≥a", "aha" itd.)
1571=Podziel d≥ugie linijki napisÛw:
1767=Minimum:
1772=przez okres:
1787=Poprawiaj ' ' na " oraz " " na "
1723=OdstÍp miedzy liniami:
1836=Odleg≥oúÊ napisÛw od krawÍdzi:
1572=Na≥Ûø na siebie napisy ktÛre sπ wyúwietlane w tym samym czasie
1793=Tekst

[250]
1725=W≥πcz
1726=Preferowane jÍzyki:
1728=Aktualny jÍzyk:
1727=Metoda wyg≥adzania:
0=VobSub
1724=VobSub
1729=Skala:
1730=Gauss'owskie zamglenie
1796=Przestrzegaj ustawieÒ pozycji napisÛw z zak≥adki Napisy
1143=Ustawienia zewnÍtrznego dodatku.\r\nPierw musi byÊ on zainstalowany øeby te opcje zadzia≥a≥y.

[179]
1324=Dyskretna kosinusowa transformata
1143=Dyskretna kosinusowa transformata jest filtrem ktÛry dla kaødego bloku 8x8 zmniejsza wybrane czÍstotliwoúci wartoúci oraz nastÍpnie przeprowadza odwrotny proces do Odwrotnej dyskretnej kosinusowej transformaty (IDCT).\r\n\r\nPrawa autorskie (2002 rok) - Tom Barry  - trbarry@trbarry.com
1917=Matryca kwantyzatora
1919=Kwantyzator H.263
1922=Kwantyzator MPEG
1920=Kwantyzator:
1325=WspÛ≥czynniki

[172]
0=Zrzut
1740=Zrzucaj co:
1741=klatkÍ
1216=Zrzut klatki
1223=Kaødej
1224=Pojedynczej:
1226=Zakres:
1228=ZrzuÊ teraz
1232=Folder
1261=Prefiks:
1262=Format:
1230=JakoúÊ:
1263=IloúÊ cyfr w nazwie:
1143=Umoøliwia zrzut klatek obrazu do pliku BMP, JPEG oraz PNG.

[173]
0=Nak≥adka
1235=Nak≥adka
1143=Dzia≥a tyko gdy zaznaczona jest opcja "Ustaw proporcjÍ pikseli w wyjúciowym typie materia≥u" w zak≥adce "Wyjúcie" oraz sprzÍt to obs≥uguje.\r\n\r\nObs≥uguje mikser nak≥adki i miksujπcy renderer obrazu 9 w trybie okienkowym.
1236=JasnoúÊ:
1137=Kontrast:
1138=Barwa:
1139=Nasycenie:
1140=OstroúÊ:

[118]
0=Defekty
1307=Autowykrywanie
1572=IloúÊ dekowanych wπtkÛw:
1943=PomiÒ klatkÍ gdy opÛünienie wynosi:
1947=PomiÒ usuwanie blokÛw w H264 po:
1946=Kontrola jakoúci
1916=Aktualne matryce kwantyzatora
1283=Opcje libavcodec
1314=Dekoduj w skali szaroúci
1544=Metoda ukrywania defektÛw:
1542=ElastycznoúÊ defektÛw:
1047=PomiÒ b≥Ídy kompresji:
1309=Stare b≥Ídy przeplotu w Xvid
1308=Stare pliki lavc msmpeg4
1311=B≥Ídy obramowania
1316=B≥Ídy chroma przeplotu MC w DivX
1315=B≥Ídy krawÍdzi
1312=B≥Ídy AMV
1317=B≥Ídy DC CLIP
1310=B≥Ídy UMP4
1313=B≥Ídy qpel w Xvid oraz DivX

[121]
0=Kolejka
1944=Kolejkowanie wyjúciowych prÛbek
1958=Uøyj kolejkowania w:
1268=OpÛünienie video
1365=Inne ustawienia
1764=koniec
1765=poczπtek
1550=Rozdziel wejúciowy przeplot jeúli wysokoúÊ > :
1552=pikseli
1143=Kolejkowanie wyjúciowych prÛbek\r\n G≥Ûwnym zadaniem tej opcji jest zmniejszenie pomijania klatek wskutek\r\n zarzπdzania wπtkami procesora\r\n Opcja ta dzia≥a tylko wtedy kiedy procesor obs≥uguje wielowπtkowoúÊ\r\n\r\nUøyj kolejkowania w\r\n Kolejkowanie jest domyúlnie wy≥πczone ze wzglÍdu na problemy ze   stabilnoúciπ\r\n Jednak jeúli chcesz dodaÊ aplikacjÍ wykorzystujπcπ wielowπtkowoúÊ oddziel\r\n pliki przez znak ;\r\n\r\nInne ustawienia\r\n OpÛünienie obrazu\r\n  Jeøeli odbiorca jest daleko od g≥oúnikÛw, moøesz wyprÛbowaÊ tπ opcjÍ
2206=W≥πcz kolejkowanie miksujπcego renderera obrazu 9 - YV12

[2112]
0=Konwersja
2123=Specyfikacja YCbCr:
2122=Kolory:
2125=Pe≥ny zakres ( luminancja : 0-255, chrominancja : 1-255 )
2124=Standard ( luminancja : 16-235, chrominancja : 16-240 )
2126=W≥asne
2118=ObciÍcie czerni:
2119=ObciÍcie bieli:
2120=ObciÍcie chrominancji:
2121=Powiπø z luminancjπ

[255]
0=Nak≥adka obrazu
1798=Nak≥adka obrazu
1799=åcieøka pliku
1802=Poziome po≥oøenie:
1803=Pionowe po≥oøenie:
1810=Po≥oøenie:
1361=Efekt:
1807=PrzeüroczystoúÊ

[261]
1858=Procenty
1859=Piksele
2068=Lewo
2069=Prawo
2070=GÛra
2071=DÛ≥
2079=Poziomo
2080=Pionowo
1862=Poziomo
1863=Pionowo
1199=Zablokuj
0=KrawÍdzie
1857=KrawÍdzie:
1860=WewnÍtrzne
1861=ZewnÍtrzne
2067=Dzielenie krawÍdzi

[225]
1829=Zapisz matrycÍ...
1828=Za≥aduj matrycÍ..
1840=Intra matryca:
1763=Inter matryca:

[180]
2021=Dodaj ürÛd≥owy obraz ffdshow
2029=Bufory klatek:
2032=Uøyj obecnego
2033=Za≥aduj
2034=Zapisz
2024=Wejúciowe palety:
2023=3:2 Pulldown:

[270]
1941=PrÛg:

[181]
0=WarpSharp
1338=WarpSharp
1339=Oryginalny WarpSharp ffdshow
1341=Zmiana chrominancji:
1343=Metoda wyg≥adzania:
1019=G≥ÍbokoúÊ warstwy:
1021=ZasiÍg:
1023=Wyg≥adzanie:
1143=aWarpSharp jest filtrem warsharp'ujπcym ktÛry dzia≥a inaczej niø inne filtry wyostrzajπce.\r\nPrawa autorskie - Marc Fauconneau
