                                    ...
                                    uint8_t *dest, uint8_t *uDest, uint8_t *vDest, uint8_t *aDest, long dstW, long chrDstW)
{
    ...
    if(!(c->flags & SWS_BITEXACT)) {
        long p= 4;
        const int16_t *src[4]= {alpSrc + dstW, lumSrc + dstW, chrSrc + chrDstW, chrSrc + VOFW + chrDstW}; // FFmpeg patch
                           ...
                           int srcSliceH, uint8_t* dst[], stride_t dstStride[], int dstYstart, int dstYend)
{
        ...
        while(lastInChrBuf < lastChrSrcY) {
            ...
            if (c->needs_hcscale)
                RENAME(hcscale)(c, chrPixBuf[ chrBufIndex ], chrDstW, src1, src2, chrSrcW, chrXInc,
                                params.methodChroma.method, //FFDShow : flags for chroma
    ...
    if (flags & SWS_CPU_CAPS_MMX)    __asm__ volatile("sfence":::"memory");
