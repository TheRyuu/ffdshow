                           ...
                           SwsFilter *srcFilter, SwsFilter *dstFilter, const double *param)
{
    ...
    if (!srcFilter) srcFilter= &dummyFilter;
     ...
     //FFDShow custom code for multithreading
...
// can't downscale !!!
...
#endif /* HAVE_MMX2 */
        {
            ...
            if (initFilter(&c->hLumFilter, &c->hLumFilterPos, &c->hLumFilterSize, c->lumXInc,
                           ...
                           /*(flags&SWS_BICUBLIN) ? (flags|SWS_BICUBIC)  : flags*/params->methodLuma.method|flags, cpu_flags,
                           srcFilter->lumH, dstFilter->lumH, /*c->param*/params->methodLuma.param) < 0)
                goto fail;
            if (initFilter(&c->hChrFilter, &c->hChrFilterPos, &c->hChrFilterSize, c->chrXInc,
                           c->chrSrcW, c->chrDstW, filterAlign, 1<<14,
                           /*(flags&SWS_BICUBLIN) ? (flags|SWS_BILINEAR) : flags*/params->methodChroma.method|flags, cpu_flags,
    ...
    memset(c->ret,0,threadCount*sizeof(int));
...

    {
     ...
     for (j=0; j<threadCount; j++)
      ...
      FF_ALLOC_OR_GOTO(&c[j], c[j].chrPixBuf, c[j].vChrBufSize*2*sizeof(int16_t*), fail);
      ...
      for (i=0; i<c[j].vLumBufSize; i++) {
          FF_ALLOCZ_OR_GOTO(&c[j], c[j].lumPixBuf[i+c[j].vLumBufSize], VOF+1, fail);
          c[j].lumPixBuf[i] = c[j].lumPixBuf[i+c[j].vLumBufSize];
      }
      for (i=0; i<c[j].vChrBufSize; i++) {
          FF_ALLOC_OR_GOTO(&c[j], c[j].chrPixBuf[i+c[j].vChrBufSize], (VOF+1)*2, fail);
          c[j].chrPixBuf[i] = c[j].chrPixBuf[i+c[j].vChrBufSize];
      }
      if (CONFIG_SWSCALE_ALPHA && c[j].alpPixBuf)
          for (i=0; i<c[j].vLumBufSize; i++) {
              FF_ALLOCZ_OR_GOTO(&c[j], c[j].alpPixBuf[i+c[j].vLumBufSize], VOF+1, fail);
              c[j].alpPixBuf[i] = c[j].alpPixBuf[i+c[j].vLumBufSize];
          }

      //try to avoid drawing green stuff between the right end and the stride end
      for (i=0; i<c[j].vChrBufSize; i++) memset(c[j].chrPixBuf[i], 64, (VOF+1)*2);
...
void sws_freeContext(SwsContext *c)
{
    ...
    for(i=0; i<c->thread_count; i++)
    {
      ...
      if (c[i].chrPixBuf)
      {
       ...
       {
        av_freep(&c[i].chrPixBuf[j]);
        c[i].chrPixBuf[j]=NULL;
       }
       av_freep(&c[i].chrPixBuf);
       c[i].chrPixBuf=NULL;
      }
