...
\
static inline void yuv ## depth ## ToY_c(uint8_t *dstY, const uint16_t *srcY, int width, uint32_t *unused) \
{ \
    int i; \
    for (i = 0; i < width; i++) \
...
int sws_thread_work_c(SwsContext *c)		// Thread func
{
	...
	return swScaleI_c(c, stp->src, stp->srcStride, stp->srcSliceY,
             ...
             int srcSliceH, uint8_t* dst[], stride_t dstStride[])
{
	...
	if (c->thread_count==1)
	{
		return swScaleI_c(c,src,srcStride,srcSliceY,
