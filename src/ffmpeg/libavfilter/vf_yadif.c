...
void yadif_init(YADIFContext *yadctx){
    ...
    yadctx->filter_line = filter_line_c;

    if (HAVE_SSSE3 && cpu_flags & AV_CPU_FLAG_SSSE3)
        yadctx->filter_line = ff_yadif_filter_line_ssse3;
    else if (HAVE_SSE && cpu_flags & AV_CPU_FLAG_SSE2)
        yadctx->filter_line = ff_yadif_filter_line_sse2;
    else if (HAVE_MMX && cpu_flags & AV_CPU_FLAG_MMX)
        yadctx->filter_line = ff_yadif_filter_line_mmx;
    ...
    for(i = yadThreadCtx->plane_start ; i <= yadThreadCtx->plane_end ; i++){
        ...
        for(y=yadThreadCtx->y_start[i] ; y < h ; y++){
            if((y ^ parity) & 1){
                ...
                int mode = y==1 || y+2==h ? 2 : yadctx->mode;
                yadctx->filter_line(dst2, prev, cur, next, w, y+1<h ? refs : -refs, y ? -refs : refs, parity ^ tff, mode);
            }else{
