...
/* ffdshow custom code */ void gradfunFilter(GradFunContext *ctx, uint8_t *dst, uint8_t *src, int width, int height, int dst_linesize, int src_linesize, int r)
{
...
/* ffdshow custom code */ int gradfunInit(GradFunContext *gf, const char *args, void *opaque)
{
    /* ffdshow custom code */ //GradFunContext *gf = ctx->priv;
    float thresh = 1.2;
    int radius = 16;
    ...
    /* ffdshow custom code */ //av_log(ctx, AV_LOG_INFO, "threshold:%.2f radius:%d\n", thresh, gf->radius);
...
static void end_frame(AVFilterLink *inlink)
{
    ...
    for (p = 0; p < 4 && inpic->data[p]; p++) {
        ...
        if (FFMIN(w, h) > 2 * r)
            /* ffdshow custom code */ gradfunFilter(gf, outpic->data[p], inpic->data[p], w, h, outpic->linesize[p], inpic->linesize[p], r);
...
AVFilter avfilter_vf_gradfun = {
    ...
    .init          = /* ffdshow custom code */ gradfunInit,
    .uninit        = uninit,
