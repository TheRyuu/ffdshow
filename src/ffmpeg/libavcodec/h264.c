                                        ...
                                        const AVCodecContext *src)
{
    ...
    h->has_to_drop_first_non_ref = h1->has_to_drop_first_non_ref; // ffdshow custom code
...
static void idr(H264Context *h)
{
    ...
    h->prev_poc_msb          = 1<<16;
    h->prev_poc_lsb          = 0;
    for (int i = 0; i < MAX_DELAYED_PIC_COUNT; i++)
        h->last_pocs[i] = INT_MIN;
}
    ...
    if (!s->context_initialized) {
        ...
        if (!HAVE_THREADS || !(s->avctx->active_thread_type & FF_THREAD_SLICE)) {
            ...
            for (i = 1; i < s->slice_context_count; i++) {
                ...
                c->cur_chroma_format_idc = h->cur_chroma_format_idc; // FFmpeg patch
                init_scan_tables(c);
                clone_tables(c, h, i);
            }
    ...
    if (h0->current_slice != 0) {
        ...
        if (s0->first_field) {
            ...
            if (!FIELD_PICTURE || s->picture_structure == last_pic_structure) {
                ...
                if (s0->current_picture_ptr->frame_num != h->frame_num || h->has_to_drop_first_non_ref == 3) { /* ffdshow custom code */
...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    h->nal_unit_type = 0; // FFmpeg patch

    if (!s->slice_context_count) // FFmpeg patch
    ...
    for (; pass <= 1; pass++) {
        ...
        for (;;) {
            ...
            if (!(s->workaround_bugs & FF_BUG_TRUNCATED))
                while (dst_length > 0 && ptr[dst_length - 1] == 0) // FFmpeg patch
                    dst_length--;
