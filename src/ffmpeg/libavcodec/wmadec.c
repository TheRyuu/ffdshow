...
static int wma_decode_block(WMADecodeContext *s)
{
    ...
    DECLARE_ALIGNED_16(float, w[BLOCK_MAX_SIZE * 2 + 16/sizeof(float)]); //GCC can't do this yet(4.0.3).
    float* window = (float*)(((long)w+15) & ~0xf);
// XXX: FIXME!! there's a bug somewhere which makes this mandatory under altivec
    ...
    for(ch = 0; ch < s->nb_channels; ch++) {
        if (s->channel_coded[ch]) {
            DECLARE_ALIGNED_16(FFTSample, o[BLOCK_MAX_SIZE * 2 + 16/sizeof(FFTSample)]);  // GCC can't do this yet(4.0.3).
            FFTSample* output = (FFTSample*)(((long)o+15) & ~0xf);
            float *ptr;
            int i, n4, index, n;
