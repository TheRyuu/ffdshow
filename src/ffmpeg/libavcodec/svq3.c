  ...
  if (!s->context_initialized) {
    ...
    if (!extradata){ //HACK: 3ivx splitter doesn't provide any extradata
      ...
      if(first_svq3_error)
       {
        first_svq3_error= 0;
        MessageBox(NULL,"The splitter failed to provide necessary information to decode SVQ3 properly. \nPlease try an alternative source filter.","ffdshow error",MB_ICONERROR|MB_OK);
       }
    }
