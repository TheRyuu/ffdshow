...
#define WHITELIST_EXE_FILENAME \
	...
	_l("Zune.exe;moviemk.exe;ACDSee10.exe;BoonPlayer.exe;CEC_MAIN.exe;msnmsgr.exe;QzoneMusic.exe;ReClockHelper.dll;YahooMessenger.exe;ACDSeePro2.exe;AlltoaviV4.exe;Funshion.exe;Internet TV.exe;MatroskaDiag.exe;QQ.exe;Tvants.exe;allradio.exe;amf_slv.exe;Camfrog Video Chat.exe;DivxToDVD.exe;dvdSanta.exe;DXEffectTester.exeFLVPlayer4Free.exe;GraphStudio.exe;InstallChecker.exe;ipod_video_converter.exe;IPODConverter.exe;KwMusic.exe;MP4Converter.exe;Mp4Player.exe;My Movies.exe;PQDVD_PSP.exe;ProgDvbNet.exe;QQLive.exe;QQMusic.exe;QvodPlayer.exe;RecordingManager.exe;RoxMediaDB10.exe;songbird.exe;SubtitleWorkshop4.exe;telewizja.exe;UUSeePlayer.exe;VideoConvert.exe;videoconverter.exe;videoenc.exe;VideoManager.exe;VideoWave9.exe;WaveChk.exe;WebMediaPlayer.exe;WinAVI 9.0.exe;WinAVI MP4 Converter.exe;WinDvr.exe;WLXPhotoGallery.exe;wmprph.exe;YahooWidgets.exe")

#define BLACKLIST_EXE_FILENAME _l("age3.exe;cm2008.exe;cmr4.exe;cmr5.exe;conquer.exe;dmc3se.exe;em3.exe;em4.exe;fable.exe;game.exe;gta-vc.exe;gta3.exe;gta_sa.exe;gta-underground.exe;guildII.exe;hl.exe;hl2.exe;juiced.exe;juiced2_hin.exe;maxpayne.exe;morrowind.exe;oblivion.exe;pes2008.exe;pes4.exe;pes5.exe;pes6.exe;questviewer.exe;residentevil3.exe;rf.exe;rf_online.bin;sacred.exe;sega rally.exe;sh3.exe;sh4.exe;sims.exe;tw2008.exe;twoworlds.exe;war3.exe;worms 4 mayhem.exe;worms3d.exe")
