...
void TsharpenSettings::createFilters(size_t filtersorder, Tfilters *filters, TfilterQueue &queue) const
{
    ...
    if (is && show) {
        if (!queue.temporary) {
            setOnChange(IDFF_sharpenMethod, filters, &Tfilters::onQueueChange);
        }
        switch (method) {
            case SHARPEN_XSHARP:
                queueFilter<TimgFilterXsharp>(filtersorder, filters, queue);
                return;
            case SHARPEN_UNSHARP:
                queueFilter<TimgFilterUnsharp>(filtersorder, filters, queue);
                return;
            case SHARPEN_MSHARP:
                queueFilter<TimgFilterMsharp>(filtersorder, filters, queue);
                return;
            case SHARPEN_ASHARP:
                queueFilter<TimgFilterAsharp>(filtersorder, filters, queue);
                return;
            case SHARPEN_MPLAYER:
                queueFilter<TimgFilterLibavcodecSharp>(filtersorder, filters, queue);
                return;
        }
    }
}
...
bool TsharpenSettings::getTip(unsigned int pageId, char_t *buf, size_t len)
{
    ...
    switch (method) {
        ...
        case 4:
            name = _l("asharp");
            capts[0] = _l("Unsharp masking threshold");
            capts[1] = _l("Adaptive sharpenning strength");
            capts[2] = _l("Block adaptive sharpenning");
            vals[0] = asharpT;
            vals[1] = asharpD;
            vals[2] = asharpB;
            break;
        case 5:
            name = _l("swscaler");
            capts[0] = _l("Luma sharpening");
            capts[1] = _l("Chroma sharpening");
            vals[0] = avcodecLuma;
    ...
    if (capts[0]) {
        strncatf(tipS, countof(tipS), _l("%s: %i"), capts[0], vals[0]);
    }
    if (capts[1]) {
        strncatf(tipS, countof(tipS), _l(", %s: %i"), capts[1], vals[1]);
    }
    if (capts[2]) {
        strncatf(tipS, countof(tipS), _l(", %s: %i"), capts[2], vals[2]);
    }
