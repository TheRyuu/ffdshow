...
TconvolverSettings::TconvolverSettings(TintStrColl *Icoll, TfilterIDFFs *filters): TfilterSettingsAudio(sizeof(*this), Icoll, filters, &idffs)
{
    ...
    memset(fileL, 0, sizeof(file));
    memset(fileR, 0, sizeof(file));
    memset(fileC, 0, sizeof(file));
    memset(fileLFE, 0, sizeof(file));
    memset(fileSL, 0, sizeof(file));
    memset(fileSR, 0, sizeof(file));
...
bool TconvolverSettings::getTip(unsigned int pageId, char_t *tipS, size_t len)
{
    ...
    if (mappingMode == 0)
        if (file[0]) {
            tsnprintf_s(tipS, len, _TRUNCATE, _l("Impulse file: %s"), file);
        } else {
            if (fileL[0]) {
                tsnprintf_s(tipS, len, _TRUNCATE, _l("%s channel impulse file: %s"), TsampleFormat::getSpeakerName(SPEAKER_FRONT_LEFT), fileL);
            }
            if (fileR[0]) {
