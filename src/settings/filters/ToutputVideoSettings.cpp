...
{
    ocsps.clear();
    if (i420  && !(hwDeinterlace)) {
        ocsps.push_back(FF_CSP_420P|FF_CSP_FLAGS_YUV_ORDER);
    }
    if (yv12  && !(hwDeinterlace)) {
    ...
    if (rgb555) {
        ocsps.push_back(FF_CSP_RGB15);
    }
    if (rgb565) {
        ocsps.push_back(FF_CSP_RGB16);
    }
}
