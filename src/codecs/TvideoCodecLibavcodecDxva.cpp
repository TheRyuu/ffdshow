...
bool TvideoCodecLibavcodecDxva::beginDecompress(TffPictBase &pict,FOURCC fcc,const CMediaType &mt,int sourceFlags)
{
 ...
 switch(dxvaCodecId)
 {
  case CODEC_ID_H264_DXVA:codecId=CODEC_ID_H264;break;
  case CODEC_ID_VC1_DXVA:codecId=CODEC_ID_VC1;break;
 }
...
const char_t* TvideoCodecLibavcodecDxva::getName(void) const
{
 if (avcodec) 
 {
  ...
  switch (nDXVAMode)
  {
   case MODE_SOFTWARE : tsnprintf_s(codecName, countof(codecName), _TRUNCATE, _l("%s (no DXVA) %s"), libname, (const char_t*)text<char_t>(avcodec->name));break;
   case MODE_DXVA1 : tsnprintf_s(codecName, countof(codecName), _TRUNCATE, _l("DXVA1 %s"), (const char_t*)text<char_t>(avcodec->name));break;
   case MODE_DXVA2 : tsnprintf_s(codecName, countof(codecName), _TRUNCATE, _l("DXVA2 %s"), (const char_t*)text<char_t>(avcodec->name));break;
  }
  return codecName;
 } 
 else
