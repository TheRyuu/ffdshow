...
LRESULT TvideoCodecX264::beginCompress(int cfgcomode,int csp,const Trect &r)
{
    ...
    if (Tconfig::cpu_flags&FF_CPU_MMXEXT  ) {
        param.cpu|=X264_CPU_MMXEXT;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSE     ) {
        param.cpu|=X264_CPU_SSE;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSE2    ) {
        param.cpu|=X264_CPU_SSE2;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSE3    ) {
        param.cpu|=X264_CPU_SSE3;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSSE3   ) {
        param.cpu|=X264_CPU_SSSE3;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSE41   ) {
        param.cpu|=X264_CPU_SSE4;
    }
    if (Tconfig::cpu_flags&FF_CPU_SSE42   ) {
    ...
    param.i_fps_num=deci->getParam2(IDFF_enc_fpsRate);
    param.i_fps_den=deci->getParam2(IDFF_enc_fpsScale);
    ...
    if (coCfg->codecId==CODEC_ID_X264_LOSSLESS) {
        ...
        if (coCfg->is_lavc_nr) {
            param.analyse.i_noise_reduction=coCfg->lavc_nr;
        }
    ...
    if (coCfg->isCreditsStart && cfgcomode==ENC_MODE::CBR) {
        ...
        switch (coCfg->credits_mode) {
            case CREDITS_MODE::QUANT:
                zone.i_qp=coCfg->limitq(coCfg->credits_quant_i);
                zone.b_force_qp=1;
                break;
            case CREDITS_MODE::PERCENT:
                zone.b_force_qp=0;
                zone.f_bitrate_factor=coCfg->credits_percent/100.0f;
                break;
        }
        zonescount++;
    }
