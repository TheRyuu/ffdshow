    ...
    if (codecId == CODEC_ID_LPCM) {
        if (lpcm20) {
            ...
            if (highDef) {
                ...
                if (fmt.nchannels == 8) {
                    reorder_channel(dst0, AF_CHANNEL_LAYOUT_LPCM_8CH_DEFAULT, AF_CHANNEL_LAYOUT_FFDSHOW_8CH_DEFAULT,
                                    (int)(sizeof(int32_t) * (dst - dst0) * 8 / fmt.blockAlign()), fmt.bitsPerSample() / 8);
                }
        ...
        samples = dst;
        numsamples = 2 * src.size() / fmt.blockAlign();
    } else if (be) {
        if (float64) {
            ...
            switch (fmt.sf) {
                case TsampleFormat::SF_PCM16:
                    swapbe((int16_t*)samples, src.size());
                    break;
                case TsampleFormat::SF_PCM24:
                    swapbe((int24_t*)samples, src.size());
                    break;
                case TsampleFormat::SF_PCM32:
                    swapbe((int32_t*)samples, src.size());
                    break;
                case TsampleFormat::SF_FLOAT32:
                    swapbe((float*)samples, src.size());
                    break;
            }
    }
