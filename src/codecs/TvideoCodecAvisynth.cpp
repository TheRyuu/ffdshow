        ...
        if (env) {
            ...
            try {
                ...
                if (val.IsClip()) {
                    ...
                    if (vi.IsRGB24()) {
                        pict.csp = csp = FF_CSP_RGB24 | FF_CSP_FLAGS_VFLIP;
                    } else if (vi.IsRGB32()) {
                        pict.csp = csp = FF_CSP_RGB32 | FF_CSP_FLAGS_VFLIP;
                    } else if (vi.IsYUY2()) {
                        pict.csp = csp = FF_CSP_YUY2;
                    } else if (vi.IsYV12()) {
                        pict.csp = csp = FF_CSP_420P;
                    }
                    r = pict.rectFull;
                    ok = true;
                }
...
void TvideoCodecAvisynth::forceOutputColorspace(const BITMAPINFOHEADER *hdr, int *ilace, TcspInfos &forcedCsps)
{
    ...
    *ilace = extradata[5];
    ...
    if (fcc) {
        forcedCsps.push_back(csp_getInfoFcc(fcc));
    }
}
bool TvideoCodecAvisynth::beginDecompress(TffPictBase &pict, FOURCC infcc, const CMediaType &mt, int sourceFlags)
{
    ...
    if (!extradata.data || extradata.size == 0) {
        return false;
    }
    if (!avisynth) {
...
HRESULT TvideoCodecAvisynth::decompress(const unsigned char *src, size_t srcLen, IMediaSample *pIn)
{
    ...
    if (csp_isYUVplanar(csp)) {
        dst[0] = (unsigned char*)frame->GetReadPtr(PLANAR_Y);
        stride[0] = frame->GetPitch(PLANAR_Y);
        dst[1] = (unsigned char*)frame->GetReadPtr(PLANAR_U);
        stride[1] = frame->GetPitch(PLANAR_U);
        dst[2] = (unsigned char*)frame->GetReadPtr(PLANAR_V);
        stride[2] = frame->GetPitch(PLANAR_V);
        dst[3] = NULL;
        stride[3] = 0;
    } else {
        dst[0] = (unsigned char*)frame->GetReadPtr();
        dst[1] = dst[2] = dst[3] = NULL;
        stride[0] = frame->GetPitch();
        stride[1] = stride[2] = stride[3] = NULL;
    }
