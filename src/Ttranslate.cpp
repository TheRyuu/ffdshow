...
Ttranslate::Tlanglist::Tlanglist(const char_t *Ipth): pth(Ipth)
{
    ...
    for (strings::iterator i = files.begin(); i != files.end(); i++) {
        char_t ext[MAX_PATH];
        extractfileext(i->c_str(), ext);
        strlwr(ext);
        char_t name[MAX_PATH];
        extractfilenameWOext(i->c_str(), name);
        char_t lang[MAX_PATH];
        extractfileext(name, lang);
        if (lang[0] != '\0') {
            *strchr(name, '.') = '\0';
        }
...
void Ttranslate::Tlanglist::getLangName(const_iterator &lang, char_t *buf, size_t buflen)
{
    ...
    if (buf[0] == '\0') {
        ff_strncpy(buf, lang->first.c_str(), buflen);
    }
    //if (f) fclose(f);
}
        ...
        for (int i = 0; i < numitems; i++) {
            ...
            *p++ = *orig++; // WORD id;
            ...
            if (extracount > 0) {
                ...
                p = lpwAlign(p + roundDiv(extracount, 2));
                orig = lpwAlign(orig + roundDiv(extracount, 2));
            }
        }
    }
