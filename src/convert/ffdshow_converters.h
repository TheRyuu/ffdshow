...
#include "TrgbPrimaries.h" // depends on YCbCr2RGBdata_common_inint and ffYCbCr_RGB_MatrixCoefficientsType
...
private:
 ...
 template<int incsp, int outcsp, int left_edge, int right_edge, int rgb_limit> static __forceinline 
  void convert_a_unit(const unsigned char* &srcY,
                      const unsigned char* &srcCb,
                      const unsigned char* &srcCr,
                      unsigned char* &dst,
                      const ptrdiff_t stride_Y,
                      const ptrdiff_t stride_CbCr,
                      const ptrdiff_t stride_dst,
 ...
 template <int incsp, int rgb_limit> void convert_translate_outcsp(const uint8_t* srcY,
              ...
              ptrdiff_t stride_dst)
  {
    switch (m_outcsp) {
    case FF_CSP_RGB32:
        convert_main_loop<incsp, FF_CSP_RGB32, rgb_limit>(srcY, srcCb, srcCr, dst, dx, dy, stride_Y, stride_CbCr, stride_dst);
        return;
    case FF_CSP_RGB24:
        convert_main_loop<incsp, FF_CSP_RGB24, rgb_limit>(srcY, srcCb, srcCr, dst, dx, dy, stride_Y, stride_CbCr, stride_dst);
        return;
    }
  }

 template <int incsp, int outcsp, int rgb_limit> void convert_main_loop(const uint8_t* srcY,
              const uint8_t* srcCb,
              const uint8_t* srcCr,
              uint8_t* dst,
              int dx,
              ...
              ptrdiff_t stride_dst)
  {
    ...
    if (incsp == FF_CSP_420P || incsp == FF_CSP_NV12) {
        // Top
        convert_a_unit<incsp,outcsp,1,0,rgb_limit>(srcYln, srcCbln, srcCrln, dstln,
                                0, 0, 0,
                                m_coeffs);
        for (int x = 1 ; x < endx ; x += 4) {
            convert_a_unit<incsp,outcsp,0,0,rgb_limit>(srcYln, srcCbln, srcCrln, dstln,
                                    0, 0, 0,
                                    m_coeffs);
        }
        convert_a_unit<incsp,outcsp,0,1,rgb_limit>(srcYln, srcCbln, srcCrln, dstln,
                                0, 0, 0,
                                m_coeffs);
        y = 1;
        endy = dy - 1;
