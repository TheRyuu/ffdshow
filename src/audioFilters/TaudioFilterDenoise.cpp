...
HRESULT TaudioFilterDenoise::process(TfilterQueue::iterator it, TsampleFormat &fmt, void *samples0, size_t numsamples, const TfilterSettingsAudio *cfg0)
{
    ...
    if (oldthreshold != cfg->threshold) {
        ...
        ethreshold = 1;
        ethreshold <<= (cfg->threshold / 2);
        if (cfg->threshold & 1) {
            ethreshold = (ethreshold * 3) / 2;
        }
        onSeek();
    }
    ...
    for (unsigned int i = 0; i < numsamples; i++)
        for (unsigned int channel = 0; channel < fmt.nchannels; channel++) {
            ...
            for (int band = 0; band < BANDS; band++) {
                ...
                safesquelch = squelchgain * HEADROOM;
                safesquelch /= SQUELCHMULT;
                out_ *= safesquelch;
                out_ /= HEADROOM;
                ...
                if (band < (BANDS - 1)) {
                    ...
                    register int32_t * pprev_, * pprev_1;
                    int j0;
                    j0 = MAXPREVS - filtcount[band];
                    pprev_ = &pprevs[j0];
                    pprev_1 = pprev_ + 1;
...
{
    for (int i = 0; i < 8; i++) {
        memset(&Channel[i], 0, sizeof(Channel[0]));
    }
    threshold[0] = 0;
