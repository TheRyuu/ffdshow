...
HRESULT TaudioFilterMixer::process(TfilterQueue::iterator it, TsampleFormat &fmt, void *samples, size_t numsamples, const TfilterSettingsAudio *cfg0)
{
    ...
    switch (fmt.sf) {
        case TsampleFormat::SF_PCM16  :
            mixerPCM16.process(fmt, (int16_t*&)samples, numsamples, cfg, &matrixPtr, &inmask, &outmask, wasChanged);
            break;
        case TsampleFormat::SF_PCM32  :
            mixerPCM32.process(fmt, (int32_t*&)samples, numsamples, cfg, &matrixPtr, &inmask, &outmask, wasChanged);
            break;
        case TsampleFormat::SF_FLOAT32:
            mixerFloat.process(fmt, (float*&)  samples, numsamples, cfg, &matrixPtr, &inmask, &outmask, wasChanged);
            break;
    }
...
{
    if (iid == IID_IaudioFilterMixer) {
        *ptr = (IaudioFilterMixer*)this;
        return S_OK;
    } else {
