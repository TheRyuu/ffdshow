...
HRESULT TaudioFilterEQ::process(TfilterQueue::iterator it, TsampleFormat &fmt, void *samples, size_t numsamples, const TfilterSettingsAudio *cfg0)
{
    ...
    switch (fmt.sf) {
        case TsampleFormat::SF_PCM16:
            eq.process((int16_t*)samples, numsamples, fmt.nchannels);
            break;
        case TsampleFormat::SF_PCM32:
            eq.process((int32_t*)samples, numsamples, fmt.nchannels);
            break;
        case TsampleFormat::SF_FLOAT32:
            eq.process((float*)samples, numsamples, fmt.nchannels);
            break;
    }
