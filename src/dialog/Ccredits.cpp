...
void TcreditsPage::start2dlg(void)
{
    ...
    setCheck(IDC_CHB_IS_CREDITS_START,cfgGet(IDFF_enc_isCreditsStart));
    enable(cfgGet(IDFF_enc_isCreditsStart),idCreditsStart);
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_ED_CREDITS_P_QUANT:
                    if (HIWORD(wParam)==EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            case IDC_ED_CREDITS_I_QUANT:
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_credits_quant_i);
                                break;
                            case IDC_ED_CREDITS_P_QUANT:
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_credits_quant_p);
                                break;
                        }
                        return TRUE;
                    }
