...
void TgenericWmv9page::cfg2dlg(void)
{
    ...
    SetDlgItemInt(m_hwnd,IDC_ED_WMV9_CRISP,cfgGet(IDFF_enc_wmv9_crisp),FALSE);
    enable(aviout,IDC_ED_WMV9_CRISP);
    enable(aviout,IDC_LBL_WMV9_CRISP);
    setCheck(IDC_CHB_WMV9_DEINT,cfgGet(IDFF_enc_wmv9_deint));
    setCheck(IDC_CHB_WMV9_IVTC,cfgGet(IDFF_enc_wmv9_ivtc));
    enable(!aviout,IDC_CHB_WMV9_IVTC);
}
...
INT_PTR TgenericWmv9page::msgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
            ...
            switch (LOWORD(wParam)) {
                case IDC_ED_WMV9_CPLX:
                    if (HIWORD(wParam)==EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            case IDC_ED_WMV9_CPLX:
                                eval(hed,0,enc?enc->cplxMax:0,IDFF_enc_wmv9_cplx);
                                break;
                        }
                        return TRUE;
                    }
        ...
        case WM_CTLCOLOREDIT: {
            ...
            switch (getId(hwnd)) {
                case IDC_ED_WMV9_CPLX:
                    ok=eval(hwnd,0,enc?enc->cplxMax:0);
                    break;
                default:
                    goto colorEnd;
            }
