...
void TDScalerPageFlt::cfg2dlg(void)
{
    ...
    if (fltflnm[0]) {
        ...
        if (settings) {
            ...
            if (hprm) {
                SetWindowPos(hprm, place, placer.left, placer.top, placer.right - placer.left, placer.bottom - placer.top, SWP_SHOWWINDOW);
            }
        }
    }
}
            ...
            switch (LOWORD(wParam)) {
                case IDC_BT_DSCALER_FLTFLNM:
                    if (HIWORD(wParam) == BN_CLICKED) {
                        ...
                        cfgGet(IDFF_dscalerFltflnm, fltflnm, MAX_PATH);
