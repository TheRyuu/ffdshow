...
{
    if (!pSettings || nSettings == 0) {
        return wnd = NULL;
    }
    parent = Iparent->m_hwnd;
    onChangeMsg = IonChangeMsg;
    bActive = IbActive;
    ...
    *p++ = 0;               // weight
    ...
    for (int i = 0; i < nSettings; i++) {
        ...
        if (s.Type == NOT_PRESENT || s.pValue == bActive) {
            continue;
            ...
            *p++ = id;         // ID
            *p++ = 0;
            nchar = nCopyAnsiToWideChar(p, _l("msctls_trackbar32"));
            p += nchar;
            nchar = nCopyAnsiToWideChar(p, text<char_t>(s.szDisplayName));
            p += nchar;
            *p++ = 0;
