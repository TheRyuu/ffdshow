...
void TblurPage::denoise3d2dlg(void)
{
    ...
    enable(isDenoise3d, IDC_CHB_BLUR_DENOISE3DHQ);
    setCheck(IDC_CHB_BLUR_DENOISE3DHQ, cfgGet(IDFF_denoise3Dhq));

    int x;
    char_t s[256];
    tsnprintf_s(s, countof(s), _TRUNCATE, _l("%s - "), _(IDC_CHB_BLUR_DENOISE3D));
    x = cfgGet(IDFF_denoise3Dluma);
    strncatf(s, countof(s), _l("%s%.2f"), _(IDC_CHB_BLUR_DENOISE3D, _l("luma:")), x / 100.0f);
    tbrSet(IDC_TBR_BLUR_DENOISE3D_LUMA, x);
    x = cfgGet(IDFF_denoise3Dchroma);
    strncatf(s, countof(s), _l(", %s%.2f"), _(IDC_CHB_BLUR_DENOISE3D, _l("chroma:")), x / 100.0f);
    tbrSet(IDC_TBR_BLUR_DENOISE3D_CHROMA, x);
    x = cfgGet(IDFF_denoise3Dtime);
    strncatf(s, countof(s), _l(", %s%.2f"), _(IDC_CHB_BLUR_DENOISE3D, _l("time:")), x / 100.0f);
    tbrSet(IDC_TBR_BLUR_DENOISE3D_TIME, x);
...
void TblurPage::avcodec2dlg(void)
{
    ...
    strncatf(s, countof(s), _l("%i")  , x);
    tbrSet(IDC_TBR_BLUR_MPLAYER1, x);
    x = cfgGet(IDFF_avcodecTNR2);
    strncatf(s, countof(s), _l(", %i"), x);
    tbrSet(IDC_TBR_BLUR_MPLAYER2, x);
    x = cfgGet(IDFF_avcodecTNR3);
    strncatf(s, countof(s), _l(", %i"), x);
    tbrSet(IDC_TBR_BLUR_MPLAYER3, x);
...
void TblurPage::swscaler2dlg(void)
{
    ...
    strncatf(s, countof(s), _l("%s%i"), _(IDC_CHB_BLUR_SWSCALER, _l("radius:")), x);
    tbrSet(IDC_TBR_BLUR_SWSCALER_RADIUS, x);
    x = cfgGet(IDFF_avcodecBlurLuma);
    strncatf(s, countof(s), _l(", %s%.2f"), _(IDC_CHB_BLUR_SWSCALER, _l("luma blur:")), x / 100.0f);
    tbrSet(IDC_TBR_BLUR_SWSCALER_LUMA, x);
    x = cfgGet(IDFF_avcodecBlurChroma);
    strncatf(s, countof(s), _l(", %s%.2f"), _(IDC_CHB_BLUR_SWSCALER, _l("chroma blur:")), x / 100.0f);
    tbrSet(IDC_TBR_BLUR_SWSCALER_CHROMA, x);
