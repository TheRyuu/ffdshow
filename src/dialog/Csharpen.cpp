...
void TsharpenPage::cfg2dlg(void)
{
    ...
    switch (sharpenMethod) {
        case 0:
            idff1 = IDFF_xsharp_strength;
            idff2 = IDFF_xsharp_threshold;
            idff3 = 0;
            break;
        case 1:
            idff1 = IDFF_unsharp_strength;
            idff2 = IDFF_unsharp_threshold;
            idff3 = 0;
            break;
        case 2:
            idff1 = IDFF_msharp_strength;
...
void TsharpenPage::xsharpen2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 0) {
        enable(1, IDC_LBL_SHARPEN2);
        enable(1, IDC_TBR_SHARPEN2);
        enable(0, IDC_LBL_SHARPEN3);
        enable(0, IDC_TBR_SHARPEN3);
        tbrSetRange(IDC_TBR_SHARPEN1, 1, 127, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_xsharp_strength), IDC_LBL_SHARPEN1);
        tbrSetRange(IDC_TBR_SHARPEN2, 0, 255, 16);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_xsharp_threshold), IDC_LBL_SHARPEN2);
    }
}
void TsharpenPage::unsharp2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 1) {
        enable(0, IDC_LBL_SHARPEN2);
        enable(0, IDC_TBR_SHARPEN2);
        enable(0, IDC_LBL_SHARPEN3);
        enable(0, IDC_TBR_SHARPEN3);
        tbrSetRange(IDC_TBR_SHARPEN1, 1, 127, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_unsharp_strength), IDC_LBL_SHARPEN1);
    }
}
void TsharpenPage::msharpen2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 2) {
        ...
        enable(0, IDC_TBR_SHARPEN3);
        tbrSetRange(IDC_TBR_SHARPEN1, 1, 127, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_msharp_strength), IDC_LBL_SHARPEN1);
        tbrSetRange(IDC_TBR_SHARPEN2, 0, 255, 16);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_msharp_threshold), IDC_LBL_SHARPEN2);
    }
    enable(sharpenMethod == 2, IDC_CHB_SHARPEN_MSHARPEN_MASK);
    setCheck(IDC_CHB_SHARPEN_MSHARPEN_MASK, cfgGet(IDFF_msharpMask));
    enable(sharpenMethod == 2, IDC_CHB_SHARPEN_MSHARPEN_HQ);
    setCheck(IDC_CHB_SHARPEN_MSHARPEN_HQ  , cfgGet(IDFF_msharpHQ));
}
void TsharpenPage::asharp2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 4) {
        enable(1, IDC_LBL_SHARPEN2);
        enable(1, IDC_TBR_SHARPEN2);
        enable(1, IDC_LBL_SHARPEN3);
        enable(1, IDC_TBR_SHARPEN3);
        tbrSetRange(IDC_TBR_SHARPEN1, -100, 3200);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_asharpT), IDC_LBL_SHARPEN1, _(IDC_LBL_SHARPEN1, _l("Unsharp masking threshold:")), 100);
        tbrSetRange(IDC_TBR_SHARPEN2,   0, 1600);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_asharpD), IDC_LBL_SHARPEN2, _(IDC_LBL_SHARPEN2, _l("Adaptive sharpenning strength:")), 100);
        tbrSetRange(IDC_TBR_SHARPEN3,   0, 400);
        tbrSet(IDC_TBR_SHARPEN3, cfgGet(IDFF_asharpB), IDC_LBL_SHARPEN3, NULL, 100);
    }
    enable(sharpenMethod == 4, IDC_CHB_SHARPEN_ASHARP_HQBF);
    setCheck(IDC_CHB_SHARPEN_ASHARP_HQBF, cfgGet(IDFF_asharpHQBF));
}
void TsharpenPage::avcodec2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 5) {
        ...
        enable(0, IDC_LBL_SHARPEN3);
        enable(0, IDC_TBR_SHARPEN3);
        tbrSetRange(IDC_TBR_SHARPEN1, 0, 400, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_avcodecSharpLuma), IDC_LBL_SHARPEN1, _(IDC_LBL_SHARPEN1, _l("Luminance sharpening:")), 100);
        tbrSetRange(IDC_TBR_SHARPEN2, 0, 400, 16);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_avcodecSharpChroma), IDC_LBL_SHARPEN2, _(IDC_LBL_SHARPEN2, _l("Chroma sharpening:")), 100);
    }
}
        ...
        case WM_HSCROLL:
            switch (getId(HWND(lParam))) {
                ...
                case IDC_TBR_SHARPEN3: {
                    if (idff1) {
                        cfgSet(idff1, tbrGet(IDC_TBR_SHARPEN1));
                    }
                    if (idff2) {
                        cfgSet(idff2, tbrGet(IDC_TBR_SHARPEN2));
                    }
                    if (idff3) {
                        cfgSet(idff3, tbrGet(IDC_TBR_SHARPEN3));
                    }
