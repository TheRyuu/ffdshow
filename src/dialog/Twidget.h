...
private:
    ...
    static LRESULT CALLBACK msgProc0(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
        ...
        switch (uMsg) {
            case WM_LBUTTONDOWN:
                return self->onLbuttonDown(hwnd, uMsg, wParam, lParam);
            case WM_LBUTTONUP  :
                return self->onLbuttonUp(hwnd, uMsg, wParam, lParam);
            case WM_RBUTTONDOWN:
                return self->onRbuttonDown(hwnd, uMsg, wParam, lParam);
            case WM_MOUSEMOVE  :
                return self->onMouseMove(hwnd, uMsg, wParam, lParam);
            case WM_KEYDOWN    :
                return self->onKeyDown(hwnd, uMsg, wParam, lParam);
            case WM_KEYUP      :
                return self->onKeyUp(hwnd, uMsg, wParam, lParam);
            case WM_CHAR       :
                return self->onChar(hwnd, uMsg, wParam, lParam);
            case WM_SYSKEYDOWN :
                return self->onSysKeyDown(hwnd, uMsg, wParam, lParam);
            case WM_VSCROLL    :
                return self->onVscroll(hwnd, uMsg, wParam, lParam);
            case WM_HSCROLL    :
                return self->onHscroll(hwnd, uMsg, wParam, lParam);
            case WM_PAINT      :
                return self->onPaint(hwnd, uMsg, wParam, lParam);
            case WM_KILLFOCUS  :
                return self->onKillFocus(hwnd, uMsg, wParam, lParam);
            case WM_GETDLGCODE :
                return self->onGetDlgCode(hwnd, uMsg, wParam, lParam);
            case WM_CONTEXTMENU:
                return self->onContextMenu(hwnd, uMsg, wParam, lParam);
            default:
                return CallWindowProc(self->oldproc, hwnd, uMsg, wParam, lParam);
        }
    }
protected:
    int id;
...
public:
    ...
    void restore(void) {
        if (ownproc) {
            SetWindowLongPtr(h, GWLP_WNDPROC, LONG_PTR(oldproc));
        }
