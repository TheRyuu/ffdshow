...
void TquantPage::type2dlg(void)
{
    ...
    for (int i=0; i<6; i++)
        switch (i) {
            ...
            case 2:
            case 3:
                if (xvid_codec(codecId)) {
                    cbxAdd(IDC_CBX_QUANT_TYPE,_(IDC_CBX_QUANT_TYPE,encQuantTypes[i]),i);
                }
                break;
            case 4:
                if (sup_customQuantTables(codecId)) {
                    cbxAdd(IDC_CBX_QUANT_TYPE,_(IDC_CBX_QUANT_TYPE,encQuantTypes[i]),i);
                }
                break;
            case 5:
    ...
    if (cnt==1) {
        ...
        if (ii==CB_ERR) {
            cfgSet(cfgGet(IDFF_enc_quant_type),(int)cbxGetItemData(IDC_CBX_QUANT_TYPE,ii=0));
        }
                    ...
                    if (HIWORD(wParam)==EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            ...
                            case IDC_ED_Q_I_MAX :
                                ...
                                break;
                            case IDC_ED_Q_P_MIN :
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_p_min );
                                break;
                            case IDC_ED_Q_P_MAX :
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_p_max );
                                break;
                            case IDC_ED_Q_B_MIN :
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_b_min );
                                break;
                            case IDC_ED_Q_B_MAX :
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_b_max );
                                break;
                            case IDC_ED_Q_MB_MIN:
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_mb_min);
                                break;
                            case IDC_ED_Q_MB_MAX:
                                eval(hed,parent->qmin,parent->qmax,IDFF_enc_q_mb_max);
                                break;
                        }
                        return TRUE;
                    }
