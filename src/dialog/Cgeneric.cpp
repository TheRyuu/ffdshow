...
{
    flags.clear();
    if (sup_globalheader(codecId)) {
        flags.push_back(std::make_tuple(_(IDC_LV_GENERIC,_l("Store global headers in extradata")),IDFF_enc_globalHeader,1,false));
    }
    if (sup_gray(codecId)) {
    ...
    if (sup_part(codecId)) {
        flags.push_back(std::make_tuple(_(IDC_LV_GENERIC,_l("Data partitioning")),IDFF_enc_part,1,false));
    }
    switch (codecId) {
        case CODEC_ID_H263P:
