...
template<> double strto(const char_t *s, char_t **e)
{
    return strtod(s, e);
}
template<> int strto(const char_t *s, char_t **e)
{
...
{
    CRect rs;
    GetWindowRect(GetDlgItem(m_hwnd, id), &rs);
    CPoint tl = rs.TopLeft();
    ScreenToClient(m_hwnd, &tl);
    CPoint br = rs.BottomRight();
...
Twindow::Tanchor::Tanchor(int Iid, int Ianchors, const Twindow &parent, const CRect &parentRect): id(Iid), anchors(Ianchors)
{
    ...
    if (anchors == (TanchorInfo::LEFT | TanchorInfo::TOP)) {
        return;
    }
