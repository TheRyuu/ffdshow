...
void ToutPage::stats2dlg(void)
{
    ...
    if (is && ff1_stats_mode&FFSTATS::READ && enc_mode!=ENC_MODE::CBR) {
        cfgSet(IDFF_enc_ff1_stats_mode,FFSTATS::UNUSED);
    }
    ...
    enable(enc_mode==ENC_MODE::CBR && is,IDC_RBT_LAVC_STATS_READ);
    enable(enc_mode==ENC_MODE::CBR && is,IDC_RBT_LAVC_STATS_RW);
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_ED_FPS_DEN:
                    if (HIWORD(wParam)==EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            case IDC_ED_ASPECTX:
                                eval(hed,0.1,255.0,aspectmode==TcoSettings::ASPECT_SAR?IDFF_enc_sarX1000:IDFF_enc_darX1000,1000.0);
                                break;
                            case IDC_ED_ASPECTY:
                                eval(hed,0.1,255.0,aspectmode==TcoSettings::ASPECT_SAR?IDFF_enc_sarY1000:IDFF_enc_darY1000,1000.0);
                                break;
                            case IDC_ED_FPS_NUM:
                                eval(hed,1,200*1000,IDFF_enc_fpsOverrideNum);
                                break;
                            case IDC_ED_FPS_DEN:
                                eval(hed,1,200*1000,IDFF_enc_fpsOverrideDen);
                                break;
                        }
                        return TRUE;
                    }
...
{
    char_t stats_flnm[MAX_PATH];
    cfgGet(IDFF_enc_ff1_stats_flnm,stats_flnm,MAX_PATH);
