...
void TwarpsharpPage::warpsharp2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 0) {
        ...
        tbrSetRange(IDC_TBR_SHARPEN1, 0, 255, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_warpsharpDepth), IDC_LBL_SHARPEN1);
        tbrSetRange(IDC_TBR_SHARPEN2, 0, 255, 16);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_warpsharpThreshold), IDC_LBL_SHARPEN2);
    }
}
void TwarpsharpPage::awarpsharp2dlg(int sharpenMethod)
{
    ...
    if (sharpenMethod == 1) {
        ...
        tbrSetRange(IDC_TBR_SHARPEN1, 0, 6400, 16);
        tbrSet(IDC_TBR_SHARPEN1, cfgGet(IDFF_awarpsharpDepth), IDC_LBL_SHARPEN1, NULL, 100.0f);
        tbrSetRange(IDC_TBR_SHARPEN2, 0, 99  , 5);
        tbrSet(IDC_TBR_SHARPEN2, cfgGet(IDFF_awarpsharpThresh), IDC_LBL_SHARPEN2, NULL, 100.0f);
        tbrSetRange(IDC_TBR_SHARPEN3, 0, 4   , 1);
        tbrSet(IDC_TBR_SHARPEN3, cfgGet(IDFF_awarpsharpBlur), IDC_LBL_SHARPEN3);
    }
            ...
            switch (getId(HWND(lParam))) {
                ...
                case IDC_TBR_SHARPEN3:
                    switch (cfgGet(IDFF_warpsharpMethod)) {
                        case 0:
                            cfgSet(IDFF_warpsharpDepth    , tbrGet(IDC_TBR_SHARPEN1));
                            cfgSet(IDFF_warpsharpThreshold, tbrGet(IDC_TBR_SHARPEN2));
                            warpsharp2dlg(0);
                            break;
                        case 1:
                            cfgSet(IDFF_awarpsharpDepth , tbrGet(IDC_TBR_SHARPEN1));
...
int TwarpsharpPage::getTbrIdff(int id, const TbindTrackbars bind)
{
    ...
    switch (id) {
        case IDC_TBR_SHARPEN1:
            return mode == 0 ? IDFF_warpsharpDepth : IDFF_awarpsharpDepth;
        case IDC_TBR_SHARPEN2:
            return mode == 0 ? IDFF_warpsharpThreshold : IDFF_awarpsharpThresh;
        case IDC_TBR_SHARPEN3:
            return mode == 0 ? 0 : IDFF_awarpsharpBlur;
        default:
            return 0;
    }
}
