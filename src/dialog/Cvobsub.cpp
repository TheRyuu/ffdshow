...
void TvobsubPage::cfg2dlg(void)
{
    ...
    if (lcnt > 0) {
        ...
        for (int i = 0; i < lcnt; i++) {
            const char_t *desc;
            deciV->getSubtitleLanguageDesc(i, &desc);
            if (desc[0]) {
...
void TvobsubPage::translate(void)
{
    ...
    cbxClear(IDC_CBX_VOBSUB_DEFLANG);
    cbxClear(IDC_CBX_VOBSUB_DEFLANG2);
    ...
    cbxSetCurSel(IDC_CBX_VOBSUB_DEFLANG, sel1);
    cbxSetCurSel(IDC_CBX_VOBSUB_DEFLANG2, sel2);
}
