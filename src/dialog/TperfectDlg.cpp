...
void TperfectDlg::init(void)
{
    ...
    graphBmp.bmiColors[0].rgbRed=255;
    graphBmp.bmiColors[0].rgbGreen=255;
    graphBmp.bmiColors[0].rgbBlue=255; //0 - background
    graphBmp.bmiColors[1].rgbRed=  0;
    graphBmp.bmiColors[1].rgbGreen=  0;
    graphBmp.bmiColors[1].rgbBlue=  0; //1 - percent bars
    graphBmp.bmiColors[2].rgbRed=  0;
    graphBmp.bmiColors[2].rgbGreen=255;
    graphBmp.bmiColors[2].rgbBlue=  0; //2 - average
    ...
    qpBmp.bmiColors[0].rgbRed=255;
    qpBmp.bmiColors[0].rgbGreen=255;
    qpBmp.bmiColors[0].rgbBlue=255; //0 - background
    qpBmp.bmiColors[1].rgbRed=  0;
    qpBmp.bmiColors[1].rgbGreen=  0;
    qpBmp.bmiColors[1].rgbBlue=  0; //1 - percent bars
    qpBmp.bmiColors[2].rgbRed=  0;
    qpBmp.bmiColors[2].rgbGreen=  0;
    qpBmp.bmiColors[1].rgbBlue=255; //2 - horiz line
    ...
    qcntBmp.bmiColors[0].rgbRed=255;
    qcntBmp.bmiColors[0].rgbGreen=255;
    qcntBmp.bmiColors[0].rgbBlue=255; //0 - background
    qcntBmp.bmiColors[1].rgbRed=  0;
    qcntBmp.bmiColors[1].rgbGreen=  0;
    qcntBmp.bmiColors[1].rgbBlue=  0; //1 - percent bars
...
void TperfectDlg::simulate(void)
{
    ...
    unsigned int qcntmax=0;
    for (q=1; q<=31; q++) if (encstats.quantCount[q]>qcntmax) {
            qcntmax=encstats.quantCount[q];
        }
    q=1;
    if (qcntmax!=0) {
        for (x=0; q<=31; q++,x+=bardx) {
        ...
        for (q=1; q<=31; q++) {
            ...
            quantsum+=q*encstats.quantCount[q];
            quantcnt+=encstats.quantCount[q];
...
INT_PTR TperfectDlg::msgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
        ...
        case WM_DESTROY: {
            ...
            qpBits=NULL;
            DestroyWindow(hTipgraph);
            DestroyWindow(hTipqp);
            DestroyWindow(hTipqcnt);
            break;
        }
        case WM_DRAWITEM:
            switch (wParam) {
