...
INT_PTR TeqPage::msgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_ED_EQ_LOWDB:
                    if (HIWORD(wParam) == EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            case IDC_ED_EQ_HIGHDB:
                                eval(hed, cfgGet(IDFF_eqLowdb) / 100.0 + 0.1, 100.0, IDFF_eqHighdb, 100.0);
                                lblsdb2dlg();
                                break;
                            case IDC_ED_EQ_LOWDB :
                                eval(hed, -100.0, cfgGet(IDFF_eqHighdb) / 100.0 - 0.1, IDFF_eqLowdb, 100.0);
                                lblsdb2dlg();
                                break;
                        }
                        return TRUE;
        ...
        case WM_CTLCOLOREDIT: {
            ...
            switch (getId(hwnd)) {
                case IDC_ED_EQ_HIGHDB:
                    ok = eval(hwnd, cfgGet(IDFF_eqLowdb) / 100.0 + 0.1, 100.0);
                    break;
                case IDC_ED_EQ_LOWDB:
                    ok = eval(hwnd, -100.0, cfgGet(IDFF_eqHighdb) / 100.0 - 0.1);
                    break;
                default:
                    goto endColor;
            }
    ...
    if (dlgGetFile(false, m_hwnd, _(-IDD_EQ, _l("Select Winamp preset file")), _l("Winamp equalizer presets (*.q1)\0*.q1\0"), _l("q1"), q1flnm, cfgGetStr(IDFF_winamp2dir), 0)) {
        ...
        if (!presets.empty()) {
            ...
            if (cmd > 0) {
                ...
                cfgSet(IDFF_eqLowdb, -20 * 100);
                cfgSet(IDFF_eqHighdb, 20 * 100);
                for (int i = 0; i < 10; i++) {
