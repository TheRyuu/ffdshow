...
void TmaskingPage::masking2dlg(void)
{
    ...
    setCheck(IDC_CHB_LUMI_MASKING,cfgGet(IDFF_enc_is_ff_lumi_masking));
    setText(IDC_ED_LUMI_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_lumi_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_lumi_masking),IDC_ED_LUMI_MASKING);
    setCheck(IDC_CHB_TEMPORAL_CPLX_MASKING,cfgGet(IDFF_enc_is_ff_temporal_cplx_masking));
    setText(IDC_ED_TEMPORAL_CPLX_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_temporal_cplx_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_temporal_cplx_masking),IDC_ED_TEMPORAL_CPLX_MASKING);
    setCheck(IDC_CHB_SPATIAL_CPLX_MASKING,cfgGet(IDFF_enc_is_ff_spatial_cplx_masking));
    setText(IDC_ED_SPATIAL_CPLX_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_spatial_cplx_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_spatial_cplx_masking),IDC_ED_SPATIAL_CPLX_MASKING);
    setCheck(IDC_CHB_P_MASKING,cfgGet(IDFF_enc_is_ff_p_masking));
    setText(IDC_ED_P_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_p_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_p_masking),IDC_ED_P_MASKING);
    setCheck(IDC_CHB_DARK_MASKING,cfgGet(IDFF_enc_is_ff_dark_masking));
    setText(IDC_ED_DARK_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_dark_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_dark_masking),IDC_ED_DARK_MASKING);
    setCheck(IDC_CHB_BORDER_MASKING,cfgGet(IDFF_enc_is_ff_border_masking));
    setText(IDC_ED_BORDER_MASKING,_l("%g"),float(cfgGet(IDFF_enc_ff_dark_masking1000)/1000.0));
    enable(is && cfgGet(IDFF_enc_is_ff_border_masking),IDC_ED_BORDER_MASKING);
    setCheck(IDC_CHB_NAQ,cfgGet(IDFF_enc_ff_naq));
    enable(deciE->isLAVCadaptiveQuant(),IDC_CHB_NAQ);
}
void TmaskingPage::elim2dlg(void)
{
    ...
    setCheck(IDC_CHB_ELIM_LUM,cfgGet(IDFF_enc_isElimLum));
    enable(is,IDC_CHB_ELIM_LUM);
    ...
    setCheck(IDC_CHB_ELIM_CHROM,cfgGet(IDFF_enc_isElimChrom));
    enable(is && !cfgGet(IDFF_enc_gray),IDC_CHB_ELIM_CHROM);
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_ED_ELIM_CHROM:
                    if (HIWORD(wParam)==EN_CHANGE && !isSetWindowText) {
                        ...
                        switch (LOWORD(wParam)) {
                            case IDC_ED_ELIM_LUM  :
                                eval(hed,0,1000,IDFF_enc_elimLumThres  ,getCheck(IDC_CHB_ELIM_LUM_DC  )?-1:1);
                                break;
                            case IDC_ED_ELIM_CHROM:
                                eval(hed,0,1000,IDFF_enc_elimChromThres,getCheck(IDC_CHB_ELIM_CHROM_DC)?-1:1);
                                break;
                        }
                        return TRUE;
                    }
