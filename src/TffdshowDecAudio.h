...
public:
    ...
    STDMETHODIMP getMixerMatrixData_(double matrix[6][6]) {
        return E_NOTIMPL;
    }
    STDMETHODIMP deliverSampleSPDIF(void *buf, size_t bufsize, int bit_rate, unsigned int sample_rate, int incRtDec) {
        return deliverSampleBistream(buf, bufsize, bit_rate, sample_rate, incRtDec, 0, 0);
    }
    STDMETHODIMP storeVolumeData_(unsigned int nchannels, const int channels[], const int volumes[]) {
        return E_NOTIMPL;
    }
    STDMETHODIMP getVolumeData_(unsigned int *nchannels, int channels[], int volumes[]) {
        return E_NOTIMPL;
    }
    STDMETHODIMP storeFFTdata_(unsigned int num, const float *fft) {
        return E_NOTIMPL;
    }
    STDMETHODIMP_(int) getFFTdataNum_(void) {
        return E_NOTIMPL;
    }
    STDMETHODIMP getFFTdata_(float *fft) {
        return E_NOTIMPL;
    }
...
private:
    ...
    public:
        ...
        STDMETHODIMP QueryInterface(REFIID riid, void **ppv) {
            return deciA->QueryInterface(riid, ppv);
        }
        STDMETHODIMP_(ULONG) AddRef() {
            return deciA->AddRef();
        }
        STDMETHODIMP_(ULONG) Release() {
            return deciA->Release();
        }

        STDMETHODIMP_(int) getVersion2(void) {
            return deciA->getVersion2();
        }
        STDMETHODIMP getMovieSource(const TaudioCodec* *moviePtr) {
            return deciA->getMovieSource(moviePtr);
        }
        ...
        STDMETHODIMP getMixerMatrixData_(double matrix[6][6]) {
            return deciA->getMixerMatrixData_(matrix);
        }
        STDMETHODIMP deliverSampleSPDIF(void *buf, size_t bufsize, int bit_rate, unsigned int sample_rate, int incRtDec) {
            return deciA->deliverSampleSPDIF(buf, bufsize, bit_rate, sample_rate, incRtDec);
        }
        STDMETHODIMP storeVolumeData_(unsigned int nchannels, const int channels[], const int volumes[]) {
            return deciA->storeVolumeData_(nchannels, channels, volumes);
        }
        STDMETHODIMP getVolumeData_(unsigned int *nchannels, int channels[], int volumes[]) {
            return deciA->getVolumeData_(nchannels, channels, volumes);
        }
        STDMETHODIMP storeFFTdata_(unsigned int num, const float *fft) {
            return deciA->storeFFTdata_(num, fft);
        }
        STDMETHODIMP_(int) getFFTdataNum_(void) {
            return deciA->getFFTdataNum_();
        }
        STDMETHODIMP getFFTdata_(float *fft) {
            return deciA->getFFTdata_(fft);
        }
        STDMETHODIMP_(unsigned int) getNumStreams2(void) {
            return deciA->getNumStreams2();
        }
        ...
        STDMETHODIMP_(unsigned int) getCurrentStream2(void) {
            return deciA->getCurrentStream2();
        }
        STDMETHODIMP setCurrentStream(unsigned int i) {
            return deciA->setCurrentStream(i);
        }
        STDMETHODIMP_(TffdshowDecAudioInputPin *) GetCurrentPin(void) {
        ...
        STDMETHODIMP inputSampleFormat(unsigned int *nchannels, unsigned int *freq) {
            return deciA->inputSampleFormat(nchannels, freq);
        }
        ...
        STDMETHODIMP currentSampleFormat(unsigned int *nchannels, unsigned int *freq, int *sampleFormat) {
            return deciA->currentSampleFormat(nchannels, freq, sampleFormat);
        }
        STDMETHODIMP_(int) getJitter(void) {
            return deciA->getJitter();
        }
        STDMETHODIMP_(TinputPin*) getInputPin(void) {
            return((TinputPin*)deciA->GetCurrentPin());
        };
        STDMETHODIMP deliverSampleBistream(void *buf, size_t bufsize, int bit_rate, unsigned int sample_rate, int incRtDec, int frame_length, int iec_length) {
            return deciA->deliverSampleBistream(buf, bufsize, bit_rate, sample_rate, incRtDec, frame_length, iec_length);
        }
        STDMETHODIMP_(CTransformOutputPin*) getOutputPin(void) {
            return deciA->getOutputPin();
        };
        STDMETHODIMP_(TsampleFormat) getOutsf(TsampleFormat &outsf) {
            return deciA->getOutsf(outsf);
        };
        STDMETHODIMP getInputTime(REFERENCE_TIME &rtStart, REFERENCE_TIME &rtStop) {
            return deciA->getInputTime(rtStart, rtStop);
        };
    } decAudio_char;
