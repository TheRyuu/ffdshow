...
int Txvid_2pass::codec_2pass_init(void)
{
    ...
    switch (config.mode) {
        ...
        case DLG_MODE_2PASS_2_EXT :
            if (!statsRead) {
                statsRead = ownStatsRead=new TxvidStats(config.stats1flnm,false);
            }
            if (!statsRead->ok) {
            ...
            if (config.mode == DLG_MODE_2PASS_2_EXT) {
                if (!statsRead2) {
                    statsRead2=ownStatsRead2 = new TxvidStats(config.stats2flnm,false);
                }
                    ...
                    for (i=1; i <= (int)(twopass.alt_curve_high*2)+1; i++) {
                        ...
                        if (newquant > 1) {
                            if (newquant != oldquant) {
                                ...
                                quant_threshs[newquant].thresh=i;
                                quant_threshs[newquant].percent=percent;
                                DEBUG2P(s);
                            }
                        }
                    }
...
Txvid_2pass::~Txvid_2pass()
{
    ...
    if (ownStatsRead2) {
        delete ownStatsRead2;
    }
    if (ownStatsWrite) {
        delete ownStatsWrite;
    }
}
...
{
    const TcoSettings *Iconfig;
    deciE->getCoSettingsPtr(&Iconfig);
    config=*Iconfig;
