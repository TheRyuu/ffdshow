    ...
    const stride_t *stride)
{
    if (first || subs->dx != prefs.dx || subs->dy != prefs.dy) {
        first = false;
        wrd.dx[0] = prefs.dx;
        wrd.dx[1] = wrd.dx[2] = prefs.dx / 2;
        wrd.dy[0] = prefs.dy;
        wrd.dy[1] = wrd.dy[2] = prefs.dy / 2;
        wrd.dxChar = wrd.dx[0];
        wrd.dyChar = wrd.dy[0];
        if (subs->dx != prefs.dx || subs->dy != prefs.dy) {
            subs->create(prefs.dx, prefs.dy);
        }
        wrd.bmpmskstride[0] = subs->stride[0];
        wrd.bmpmskstride[1] = subs->stride[1];
        wrd.bmpmskstride[2] = subs->stride[2];
    }
    ...
    wrd.bmp[0] = subs->img[0];
    wrd.bmp[1] = subs->img[1];
    wrd.bmp[2] = subs->img[2];
    wrd.msk[0] = subs->alpha[0];
    wrd.msk[1] = subs->alpha[1];
    wrd.msk[2] = subs->alpha[1];
    if (wasChange) {
