...
HRESULT TimgFilterPerspective::process(TfilterQueue::iterator it, TffPict &pict, const TfilterSettingsVideo *cfg0)
{
    ...
    if (!pv || !cfg->equal(old)) {
        ...
        ref[0][0] = (old.x1 * dx1[0]) / 100;
        ref[0][1] = (old.y1 * dy1[0]) / 100;
        ref[1][0] = (old.x2 * dx1[0]) / 100;
        ref[1][1] = (old.y2 * dy1[0]) / 100;
        ref[2][0] = (old.x3 * dx1[0]) / 100;
        ref[2][1] = (old.y3 * dy1[0]) / 100;
        ref[3][0] = (old.x4 * dx1[0]) / 100;
        ref[3][1] = (old.y4 * dy1[0]) / 100;
    ...
    switch (interpolation) {
        case 0:
            resampleNone(dstY, stride2[0], srcY, stride1[0], dx1[0], dy1[0], pict.cspInfo.shiftX[0], pict.cspInfo.shiftY[0], 0);
            ...
            break;
        case 1:
            resampleLinear(dstY, stride2[0], srcY, stride1[0], dx1[0], dy1[0], pict.cspInfo.shiftX[0], pict.cspInfo.shiftY[0], 0);
            ...
            break;
        case 2:
            resampleCubic(dstY, stride2[0], srcY, stride1[0], dx1[0], dy1[0], pict.cspInfo.shiftX[0], pict.cspInfo.shiftY[0], 0);
