    ...
    for (;x<cnt3;x+=_mm::size/2)
     {
      ...
      dst8=_mm::srai_pi16(dst8,7);
      _mm::store2(dst[0]+x,_mm::packs_pu16(dst8,m0));
     }
    for (;x<cnt;x++)
     {
...
void TimgFilterBitmap::TrenderedSubtitleLineBitmap::print(int startx, int starty /* not used */, unsigned int dx[3],int dy1[3],unsigned char *dstLn[3],const stride_t stride[3],const unsigned char *bmp[3],const unsigned char *msk[3]) const
{
