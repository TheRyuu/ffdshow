...
private:
    template<uint32_t d, uint32_t r, uint32_t g, uint32_t b> struct makecol_depth {
        static const uint32_t value = r + (g << 8) + (b << 16);
    };
    ...
    static int GET_RESULT(unsigned long A, unsigned long B, unsigned long C, unsigned long D) {
