...
public:
    ...
    HRESULT convertOutputSample(const TffPict &pict,uint64_t dstcsp,unsigned char *dst[4],int dstStride[4],LONG &dstSize,const ToutputVideoSettings *cfg);
    ...
    void setStopAtSubtitles(bool stop) {
        stopAtSubtitles = stop;
    }
    bool getStopAtSubtitles(void) {
        return stopAtSubtitles;
    }
...
private:
    ...
    Tfilter *expand;
    TresizeAspectSettings *resizeSettingsDV;
    Tfilter *resizeDV;
protected:
    virtual void onFirstProcess(const TpresetVideo *cfg);
