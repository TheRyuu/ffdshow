...
HRESULT TimgFilterAsharp::process(TfilterQueue::iterator it, TffPict &pict, const TfilterSettingsVideo *cfg0)
{
    if (is(pict, cfg0)) {
        ...
        if (oldAsharpT != cfg->asharpT || oldAsharpD != cfg->asharpD || oldAsharpB != cfg->asharpB || oldAsharpHQBF != cfg->asharpHQBF) {
            oldAsharpT = cfg->asharpT;
            oldAsharpD = cfg->asharpD;
            oldAsharpB = cfg->asharpB;
            oldAsharpHQBF = cfg->asharpHQBF;
        ...
        if (!aline) {
            aline = (unsigned char*)aligned_malloc(pictRect.dx * 2);
        }
        unsigned char *dst;
        ...
        if (oldAsharpHQBF) {
            lastX2 = (unsigned char*)_alloca(dy1[0]);
            lastX1 = (unsigned char*)_alloca(dy1[0]);
...
TimgFilterLibavcodecSharp::TimgFilterLibavcodecSharp(IffdshowBase *Ideci, Tfilters *Iparent): TimgFilter(Ideci, Iparent)
{
    ...
    swsc = NULL;
    swsf = NULL;
    oldavcodecsharpenluma = oldavcodecsharpenchroma = -1;
}
...
HRESULT TimgFilterLibavcodecSharp::process(TfilterQueue::iterator it, TffPict &pict, const TfilterSettingsVideo *cfg0)
{
    if (is(pict, cfg0)) {
        ...
        if (!swsc || oldavcodecsharpenluma != cfg->avcodecLuma || oldavcodecsharpenchroma != cfg->avcodecChroma) {
            oldavcodecsharpenluma = cfg->avcodecLuma;
            oldavcodecsharpenchroma = cfg->avcodecChroma;
            if (!libavcodec) {
...
HRESULT TimgFilterUnsharp::process(TfilterQueue::iterator it, TffPict &pict, const TfilterSettingsVideo *cfg0)
{
    if (is(pict, cfg0)) {
        ...
        const unsigned char *srcL = src_ + stride1[0], *srcR = srcL + dx1[0] - 1;
        unsigned char *dstL = dst_ + stride2[0], *dstR = dstL + dx1[0] - 1;
...
HRESULT TimgFilterMsharp::process(TfilterQueue::iterator it, TffPict &pict, const TfilterSettingsVideo *cfg0)
{
    if (is(pict, cfg0)) {
        ...
        if (!blur3x3) {
            blur3x3 = new T3x3blurSWS(deci, dx1[0], dy1[0]);
        }
        if (!blur) {
